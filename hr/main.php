...
function bntm_hr_my_profile_view($user_id)
{
    ...
    <div class="bntm-form-section">
        <h3>My Profile</h3>
        <p style="color:#6b7280; margin-bottom:20px;">Update your personal details below. Your role and payroll settings are
            managed by your administrator.</p>
        ...
        <form id="my-profile-form" class="bntm-form">
            ...
            <div style="background:#fefce8; border:1px solid #fde047; border-radius:8px; padding:20px; margin-bottom:20px;">
                ...
                <div class="bntm-form-row">
                    ...
                    <div class="bntm-form-group" id="profile-confirm-group" style="display:none;">
                        ...
                        <input type="password" name="confirm_password" id="profile-confirm-password"
                ...
                <div class="bntm-form-row">
                    <div class="bntm-form-group">
                        ...
                        <input type="tel" name="phone"
                            value="<?php echo esc_attr(get_user_meta($user_id, 'bntm_phone', true)); ?>"
                            placeholder="e.g., 09XX-XXX-XXXX" />
                    </div>
                    <div class="bntm-form-group">
                        <label>Date of Birth</label>
                        <input type="date" name="dob"
                            value="<?php echo esc_attr(get_user_meta($user_id, 'bntm_dob', true)); ?>" />
                    </div>
                </div>
                <div class="bntm-form-group">
                    <label>Home Address</label>
                    <textarea name="address" rows="2"
                        placeholder="Enter your home address"><?php echo esc_textarea(get_user_meta($user_id, 'bntm_address', true)); ?></textarea>
                </div>
            </div>
                ...
                <div class="bntm-form-row">
                    <div class="bntm-form-group">
                        ...
                        <input type="text" name="emergency_name"
                            value="<?php echo esc_attr(get_user_meta($user_id, 'bntm_emergency_contact_name', true)); ?>"
                            placeholder="Full name" />
                    </div>
                    <div class="bntm-form-group">
                        <label>Contact Phone</label>
                        <input type="tel" name="emergency_phone"
                            value="<?php echo esc_attr(get_user_meta($user_id, 'bntm_emergency_contact_phone', true)); ?>"
                            placeholder="e.g., 09XX-XXX-XXXX" />
                    </div>
                </div>
            </div>
            ...
            <div style="background:#f0fdf4; border:1px solid #bbf7d0; border-radius:8px; padding:20px; margin-bottom:20px;">
                <h4 style="margin:0 0 15px 0; color:#166534;">🏢 Employment Information <small
                        style="font-weight:normal; font-size:12px; color:#6b7280;">(Read-only — contact your manager to
                        update)</small></h4>
                <div class="bntm-form-row">
                    <div class="bntm-form-group">
                        <label>Department</label>
                        <input type="text"
                            value="<?php echo esc_attr(get_user_meta($user_id, 'bntm_department', true) ?: '—'); ?>"
                            disabled style="background:#e9ecef; cursor:not-allowed;" />
                    </div>
                    <div class="bntm-form-group">
                        <label>Position</label>
                        <input type="text"
                            value="<?php echo esc_attr(get_user_meta($user_id, 'bntm_position', true) ?: '—'); ?>" disabled
                ...
                <div class="bntm-form-row">
                    <div class="bntm-form-group">
                        <label>Role</label>
                        <input type="text"
                            value="<?php echo esc_attr(ucfirst(get_user_meta($user_id, 'bntm_role', true) ?: '—')); ?>"
                            disabled style="background:#e9ecef; cursor:not-allowed;" />
                    </div>
                    <div class="bntm-form-group">
    ...
    <div class="bntm-kiosk-container" style="max-width: 500px; margin: 50px auto; text-align: center;">
        ...
        <div class="bntm-form-section">
            <div class="bntm-form-group">
                ...
                <input type="text" id="kiosk-employee-id" placeholder="Enter your ID or PIN"
...
function bntm_hr_settings_view($can_manage)
{
    ...
    <div class="bntm-form-section">
        ...
        <div id="roles-list">
            <?php if ($roles_array):
                foreach ($roles_array as $key => $role): ?>
                    <div class="role-item"
                        style="background: #f9fafb; padding: 20px; border-radius: 8px; margin-bottom: 15px; border: 2px solid #e5e7eb;">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px;">
                            <div style="flex: 1;">
                                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                                    <strong
                                        style="font-size: 16px; color: #1f2937;"><?php echo esc_html($role['label']); ?></strong>
                                    <span
                                        style="background: #dbeafe; color: #1e40af; padding: 2px 8px; border-radius: 4px; font-size: 12px; font-weight: 600;"><?php echo esc_html($key); ?></span>
                                </div>
                                ...
                                <div
                                    style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; background: white; padding: 12px; border-radius: 6px; margin-top: 10px;">
                                    <div>
                                        <div style="font-size: 11px; color: #6b7280; text-transform: uppercase;">💰 Hourly Rate
                                        </div>
                                        <div style="font-size: 16px; font-weight: 700; color: #059669;">
                                            ₱<?php echo number_format(floatval($role['hourly_rate'] ?? $hourly_rate), 2); ?></div>
                                    </div>
                                    <div>
                                        <div style="font-size: 11px; color: #6b7280; text-transform: uppercase;">⏰ Max Daily Hours
                                        </div>
                                        <div style="font-size: 16px; font-weight: 700; color: #2563eb;">
                                            <?php echo floatval($role['work_hours'] ?? $work_hours); ?>h</div>
                                    </div>
                                    <div>
                                        ...
                                        <div style="font-size: 16px; font-weight: 700; color: #f59e0b;">
                                            <?php echo floatval($role['lunch_break_hours'] ?? $lunch_break_hours); ?>h</div>
                                    </div>
                                </div>
                            </div>
                            <div style="display: flex; gap: 8px;">
                                <button class="bntm-btn-small edit-role" data-key="<?php echo esc_attr($key); ?>">Edit</button>
                                <button class="bntm-btn-small bntm-btn-danger delete-role"
                                    data-key="<?php echo esc_attr($key); ?>">Delete</button>
                            </div>
                        </div>
                    </div>
                <?php endforeach; else: ?>
                <p style="color: #6b7280;">No custom roles defined yet.</p><?php endif; ?>
        </div>
        <div style="margin-top: 20px; border-top: 2px solid #e5e7eb; padding-top: 20px;">
            ...
            <div class="bntm-form-row">
                <div class="bntm-form-group"><label>Role Key (lowercase, no spaces) *</label><input type="text"
                        id="new-role-key" placeholder="e.g., supervisor" /><small>Cannot be changed after creation</small>
                </div>
                <div class="bntm-form-group"><label>Role Label *</label><input type="text" id="new-role-label"
                        placeholder="e.g., Supervisor" /></div>
            </div>
            <div class="bntm-form-group">
                <label>Module Access *</label>
                <p style="font-size: 13px; color: #6b7280; margin-bottom: 10px;">Select which modules this role can access:
                </p>
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 8px;">
                    <?php foreach ($available_modules as $module_slug => $module_data): ?>
                        <label
                            style="display: flex; align-items: center; padding: 8px; background: #f9fafb; border-radius: 4px; cursor: pointer;"><input
                                type="checkbox" class="role-module" value="<?php echo esc_attr($module_slug); ?>"
                                style="margin-right: 8px; width: unset;" /><span><?php echo esc_html($module_data['name']); ?></span></label>
                    <?php endforeach; ?>
                </div>
            </div>
            <div
                style="background: #fef3c7; border: 2px solid #fde047; border-radius: 8px; padding: 20px; margin-top: 20px;">
                <h4 style="margin-top: 0; color: #854d0e;">💼 Payroll Settings for This Role</h4>
                <div class="bntm-form-row">
                    <div class="bntm-form-group"><label>Hourly Rate (₱) *</label><input type="number" id="role-hourly-rate"
                            step="0.01" min="0" placeholder="<?php echo $hourly_rate; ?>" /><small>Default:
                            ₱<?php echo number_format($hourly_rate, 2); ?>/hour</small></div>
                    <div class="bntm-form-group"><label>Max Work Hours per Day *</label><input type="number"
                            id="role-work-hours" step="0.5" min="1" max="24"
                            placeholder="<?php echo $work_hours; ?>" /><small>Default: <?php echo $work_hours; ?>
                            hours</small></div>
                    <div class="bntm-form-group"><label>Lunch Break Duration (hours) *</label><input type="number"
                            id="role-lunch-break" step="0.5" min="0" max="2"
                            placeholder="<?php echo $lunch_break_hours; ?>" /><small>Default:
                            <?php echo $lunch_break_hours; ?> hour(s)</small></div>
                </div>
            </div>
    ...
    <div class="bntm-form-section">
        ...
        <div class="bntm-form-row">
            <div class="bntm-form-group"><label>Default Work Hours per Day</label><input type="number" id="hr-work-hours"
                    value="<?php echo esc_attr($work_hours); ?>" step="0.5" /></div>
            <div class="bntm-form-group"><label>Default Hourly Rate</label><input type="number" id="hr-hourly-rate"
                    value="<?php echo esc_attr($hourly_rate); ?>" step="0.01" /></div>
            <div class="bntm-form-group"><label>Default Lunch Break (hours)</label><input type="number"
                    id="hr-lunch-break-hours" value="<?php echo esc_attr($lunch_break_hours); ?>" step="0.5" min="0"
                    max="2" /></div>
            <div class="bntm-form-group"><label>Default Overtime Rate</label><input type="number" id="hr-ot-rate"
                    value="<?php echo esc_attr($ot_rate); ?>" step="0.1" min="0" max="2" /></div>
        </div>
        ...
        <div id="deductions-list">
            ...
            if ($deductions_array):
                foreach ($deductions_array as $key => $deduction): ?>
                    <div class="role-item"
                        style="background: #f9fafb; padding: 15px; border-radius: 6px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <strong><?php echo esc_html($deduction['name']); ?></strong>
                            <div style="font-size: 13px; color: #6b7280; margin-top: 5px;">
                                Type: <?php echo esc_html(ucfirst($deduction['type'])); ?> |
                                <?php if ($deduction['type'] === 'percentage'): ?>Rate:
                                    <?php echo esc_html($deduction['value']); ?>%
                                <?php else: ?>Amount: ₱<?php echo esc_html(number_format($deduction['value'], 2)); ?><?php endif; ?>
                                <?php if (!empty($deduction['description'])): ?> |
                                    <?php echo esc_html($deduction['description']); ?>            <?php endif; ?>
                            </div>
                        </div>
                        <button class="bntm-btn-small bntm-btn-danger delete-deduction"
                            data-key="<?php echo esc_attr($key); ?>">Delete</button>
                    </div>
                <?php endforeach; else: ?>
                <p style="color: #6b7280;">No deductions configured yet.</p><?php endif; ?>
        </div>
        <div style="margin-top: 20px; border-top: 1px solid #e5e7eb; padding-top: 20px;">
            <h4>Add New Deduction</h4>
            <div class="bntm-form-group"><label>Deduction Name *</label><input type="text" id="deduction-name"
                    placeholder="e.g., SSS, PhilHealth, Pag-IBIG, Tax" /></div>
            <div class="bntm-form-row">
                <div class="bntm-form-group"><label>Type *</label><select id="deduction-type">
                        <option value="percentage">Percentage (%)</option>
                        <option value="fixed">Fixed Amount (₱)</option>
                    </select></div>
                <div class="bntm-form-group"><label>Value *</label><input type="number" id="deduction-value" step="0.01"
                        placeholder="e.g., 3.63 or 100.00" /></div>
            </div>
            <div class="bntm-form-group"><label>Description (Optional)</label><input type="text" id="deduction-description"
                    placeholder="e.g., Mandatory government contribution" /></div>
...
function bntm_ajax_hr_add_role()
{
    ...
    if (in_array($role_key, ['staff', 'manager', 'owner'])) {
...
function bntm_hr_dashboard_view($user_id, $can_manage)
{
    ...
    if ($can_manage):
            ...
            <?php if ($kiosk_url): ?>
                <div class="bntm-form-section" style="background: #eff6ff; border-left: 4px solid #3b82f6;">
                    ...
                    <div style="display: flex; align-items: center; gap: 15px; flex-wrap: wrap;">
                        <input type="text" id="kiosk-url" value="<?php echo esc_url($kiosk_url); ?>" readonly
                            style="flex: 1; min-width: 300px; padding: 10px; border: 1px solid #d1d5db; border-radius: 6px; background: white;">
        ...
        <div class="bntm-form-section">
            ...
            <?php $recent_leaves = $wpdb->get_results("SELECT * FROM {$prefix}hr_leave_requests ORDER BY created_at DESC LIMIT 5");
            if ($recent_leaves): ?>
                            ...
                            <?php foreach ($recent_leaves as $leave):
                                ...
                                <tr>
                                    ...
                                    <td><span class="<?php echo $status_class; ?>"
                                            style="padding: 4px 8px; border-radius: 4px; display: inline-block;"><?php echo esc_html($leave->status); ?></span>
                                    </td>
                                    <td><?php if ($leave->status === 'pending'): ?><button class="bntm-btn-small approve-leave"
                                                data-id="<?php echo $leave->id; ?>">Approve</button><button
                                                class="bntm-btn-small bntm-btn-danger reject-leave"
                                                data-id="<?php echo $leave->id; ?>">Reject</button><?php endif; ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
            ...
            <?php else: ?>
                <p>No recent leave requests.</p><?php endif; ?>
        </div>
        <div class="bntm-form-section">
            <h3>Recent Overtime Requests</h3>
            <?php $recent_overtime = $wpdb->get_results("SELECT o.*, u.display_name FROM {$prefix}hr_overtime o LEFT JOIN {$wpdb->users} u ON o.employee_id = u.ID ORDER BY o.created_at DESC LIMIT 5");
            if ($recent_overtime): ?>
                <div class="bntm-table-wrapper">
                    <table class="bntm-table">
                        <thead>
                        ...
                        <tbody>
                            <?php foreach ($recent_overtime as $ot):
                                ...
                                <tr>
                                    ...
                                    <td><span class="<?php echo $status_class; ?>"
                                            style="padding: 4px 8px; border-radius: 4px; display: inline-block;"><?php echo esc_html(ucfirst($ot->status)); ?></span>
                                    </td>
                                    <td><?php if ($ot->status === 'pending'): ?><button class="bntm-btn-small approve-overtime"
                                                data-id="<?php echo $ot->id; ?>">Approve</button><button
                                                class="bntm-btn-small bntm-btn-danger reject-overtime"
                                                data-id="<?php echo $ot->id; ?>">Reject</button><?php endif; ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
            ...
            <?php else: ?>
                <p>No recent overtime requests.</p><?php endif; ?>
        </div>
        <div class="bntm-form-section">
            <h3>Recent Missing Logs</h3>
            <?php $recent_missing = $wpdb->get_results("SELECT m.*, u.display_name FROM {$prefix}hr_missing_logs m LEFT JOIN {$wpdb->users} u ON m.employee_id = u.ID ORDER BY m.created_at DESC LIMIT 5");
            if ($recent_missing): ?>
                <div class="bntm-table-wrapper">
                    <table class="bntm-table">
                        <thead>
                        ...
                        <tbody>
                            <?php foreach ($recent_missing as $log):
                                ...
                                <tr>
                                    ...
                                    <td><span class="<?php echo $status_class; ?>"
                                            style="padding: 4px 8px; border-radius: 4px; display: inline-block;"><?php echo esc_html(ucfirst($log->status)); ?></span>
                                    </td>
                                    <td><?php if ($log->status === 'pending'): ?><button class="bntm-btn-small approve-missing"
                                                data-id="<?php echo $log->id; ?>">Approve</button><button
                                                class="bntm-btn-small bntm-btn-danger reject-missing"
                                                data-id="<?php echo $log->id; ?>">Reject</button><?php endif; ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
            ...
            <?php else: ?>
                <p>No recent missing logs.</p><?php endif; ?>
        </div>
    <?php else:
        ...
        <div class="bntm-form-section">
            ...
            <div class="bntm-form-group"><button id="quick-clock-in" class="bntm-btn-primary">Clock In</button><button
                    id="quick-clock-out" class="bntm-btn-secondary">Clock Out</button>
        ...
        <div class="bntm-form-section">
            ...
            <?php if ($today_attendance): ?>
                ...
                <?php else: ?>
                    <p><em>Currently clocked in</em></p><?php endif; ?>
            <?php else: ?>
                <p>No attendance record for today.</p><?php endif; ?>
        </div>
        <div class="bntm-form-section">
            <h3>Leave Summary</h3>
...
function bntm_hr_employees_view($can_manage)
{
    ...
    foreach ($roles as $role_key => $role_label) {
        if (isset($roles_array[$role_key])) {
            $role_settings[$role_key] = ['hourly_rate' => $roles_array[$role_key]['hourly_rate'] ?? $default_hourly_rate, 'work_hours' => $roles_array[$role_key]['work_hours'] ?? $default_work_hours, 'lunch_break_hours' => $roles_array[$role_key]['lunch_break_hours'] ?? $default_lunch_break];
        } else {
            $role_settings[$role_key] = ['hourly_rate' => $default_hourly_rate, 'work_hours' => $default_work_hours, 'lunch_break_hours' => $default_lunch_break];
        }
    }
    ?>
    <div class="bntm-form-section">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            ...
            <button id="add-employee-btn" class="bntm-btn-primary" <?php echo $limit_reached ? 'disabled' : ''; ?>>+ Add
                Employee<?php echo $limit_text; ?></button>
        </div>
        <?php if ($limit_reached): ?>
            <div
                style="background: #fee2e2; border: 1px solid #fca5a5; padding: 10px; border-radius: 4px; margin-bottom: 15px;">
                <strong>⚠️ Employee Limit Reached:</strong> Maximum of <?php echo $employee_limit; ?> employees allowed.</div>
        <?php endif; ?>
        <!-- Add Employee Modal -->
        <div id="employee-modal"
            style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center;">
            <div
                style="background: white; padding: 30px; border-radius: 8px; max-width: 600px; width: 90%; max-height: 90vh; overflow-y: auto;">
                <h3 style="margin-top: 0;">Add New Employee</h3>
                <form id="add-employee-form" class="bntm-form">
                    <div class="bntm-form-group"><label>First Name *</label><input type="text" name="first_name" required />
                    </div>
                    <div class="bntm-form-group"><label>Last Name *</label><input type="text" name="last_name" required />
                    </div>
                    <div class="bntm-form-group"><label>Email *</label><input type="email" name="email" required /></div>
                    <div class="bntm-form-group"><label>Username *</label><input type="text" name="username"
                            required /><small>Used for login</small></div>
                    <div class="bntm-form-group"><label>Password *</label><input type="password" name="password"
                            id="add-password" required /></div>
                    <div class="bntm-form-group"><label>Confirm Password *</label><input type="password"
                            name="confirm_password" id="add-confirm-password" required /><small id="add-password-match"
                            style="color: red; display: none;">Passwords do not match</small></div>
                    <div class="bntm-form-group"><label>Role *</label><select name="role" id="add-role-select"
                            required><?php foreach ($roles as $key => $label): ?>
                                <option value="<?php echo esc_attr($key); ?>"><?php echo esc_html($label); ?></option>
                            <?php endforeach; ?>
                        </select></div>
                    <div id="add-role-info"
                        style="background: #fef3c7; border: 2px solid #fde047; border-radius: 6px; padding: 15px; margin-bottom: 15px; display: none;">
                        ...
                        <div style="font-size: 12px; color: #78716c; margin-top: 8px;"><em>You can override these values
                                below</em></div>
                    </div>
                    <div class="bntm-form-row">
                        <div class="bntm-form-group"><label>Phone</label><input type="tel" name="phone" /></div>
                        <div class="bntm-form-group"><label>Date of Birth</label><input type="date" name="dob" /></div>
                    </div>
                    <div class="bntm-form-group"><label>Address</label><textarea name="address" rows="2"></textarea></div>
                    <div class="bntm-form-row">
                        <div class="bntm-form-group"><label>Department</label><input type="text" name="department"
                                placeholder="e.g., Sales, IT, HR" /></div>
                        <div class="bntm-form-group"><label>Position</label><input type="text" name="position"
                                placeholder="e.g., Sales Manager" /></div>
                    </div>
                    <div class="bntm-form-row">
                        <div class="bntm-form-group"><label>Hire Date</label><input type="date" name="hire_date"
                                value="<?php echo current_time('Y-m-d'); ?>" /></div>
                        <div class="bntm-form-group"><label>Hourly Rate (₱)</label><input type="number" name="hourly_rate"
                                id="add-hourly-rate" step="0.01" placeholder="Auto-filled from role" /><small>Leave empty to
                                use role default</small></div>
                    </div>
                    <div class="bntm-form-group"><label>Employee PIN (for Kiosk)</label><input type="text" name="pin"
                            maxlength="6" placeholder="4-6 digit PIN" /><small>Used for clock in/out at kiosk</small></div>
                    <div class="bntm-form-group"><label>Emergency Contact Name</label><input type="text"
                            name="emergency_name" /></div>
                    <div class="bntm-form-group"><label>Emergency Contact Phone</label><input type="tel"
                            name="emergency_phone" /></div>
                    <div
                        style="background: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 8px; padding: 15px; margin-bottom: 15px;">
                        <div style="font-weight: 600; color: #166534; margin-bottom: 10px;">📋 Tax & Benefits Information
                        </div>
                        <div class="bntm-form-row">
                            <div class="bntm-form-group"><label>TIN (Tax ID Number)</label><input type="text" name="tin"
                                    placeholder="e.g., 123-456-789-000" /></div>
                            <div class="bntm-form-group"><label>SSS Number</label><input type="text" name="sss"
                                    placeholder="e.g., XX-XXXXXXX-X" /></div>
                        </div>
                        <div class="bntm-form-row">
                            <div class="bntm-form-group"><label>Employee Number</label><input type="text"
                                    name="employee_number" placeholder="e.g., EMP-001" /></div>
                            <div class="bntm-form-group"><label>Pag-IBIG Number</label><input type="text" name="pagibig"
                                    placeholder="e.g., XXXX-XXXX-XXXX" /></div>
                        </div>
                    </div>
                    <div style="display: flex; gap: 10px; margin-top: 20px;"><button type="submit" class="bntm-btn-primary"
                            id="add-submit-btn">Add Employee</button><button type="button" id="close-employee-modal"
                            class="bntm-btn-secondary">Cancel</button></div>
        ...
        <div id="edit-employee-modal"
            ...
            <div
                ...
                <form id="edit-employee-form" class="bntm-form">
                    ...
                    <div class="bntm-form-group"><label>Email *</label><input type="email" name="email" required /></div>
                    <div class="bntm-form-group"><label>New Password (leave blank to keep current)</label><input
                            type="password" name="password" id="edit-password" /><small>Only fill if you want to change the
                            password</small></div>
                    <div class="bntm-form-group" id="edit-confirm-group" style="display: none;"><label>Confirm New Password
                            *</label><input type="password" name="confirm_password" id="edit-confirm-password" /><small
                            id="edit-password-match" style="color: red; display: none;">Passwords do not match</small></div>
                    <div class="bntm-form-group"><label>Role *</label><select name="role" id="edit-role-select"
                            required><?php foreach ($roles as $key => $label): ?>
                                <option value="<?php echo esc_attr($key); ?>"><?php echo esc_html($label); ?></option>
                            <?php endforeach; ?>
                        </select></div>
                    <div id="edit-role-info"
                        style="background: #fef3c7; border: 2px solid #fde047; border-radius: 6px; padding: 15px; margin-bottom: 15px; display: none;">
                        <div style="font-weight: 600; color: #854d0e; margin-bottom: 8px;">💼 Role Default Settings</div>
                        <div style="font-size: 13px; color: #92400e;">
                        ...
                        <div style="font-size: 12px; color: #78716c; margin-top: 8px;"><button type="button"
                                id="apply-role-defaults" class="bntm-btn-small" style="margin-top: 5px;">Apply Role
                                Defaults</button></div>
                    </div>
                    <div class="bntm-form-row">
                        <div class="bntm-form-group"><label>Phone</label><input type="tel" name="phone" /></div>
                    ...
                    <div class="bntm-form-group"><label>Address</label><textarea name="address" rows="2"></textarea></div>
                    <div class="bntm-form-row">
                        <div class="bntm-form-group"><label>Department</label><input type="text" name="department" /></div>
                        <div class="bntm-form-group"><label>Position</label><input type="text" name="position" /></div>
                    </div>
                    <div class="bntm-form-row">
                        <div class="bntm-form-group"><label>Hire Date</label><input type="date" name="hire_date" /></div>
                        <div class="bntm-form-group"><label>Hourly Rate (₱)</label><input type="number" name="hourly_rate"
                                id="edit-hourly-rate" step="0.01" /><small>Leave empty to use role default</small></div>
                    </div>
                    <div class="bntm-form-group"><label>Employee PIN (for Kiosk)</label><input type="text" name="pin"
                            maxlength="6" /></div>
                    <div class="bntm-form-group"><label>Emergency Contact Name</label><input type="text"
                    ...
                    <div class="bntm-form-group"><label>Emergency Contact Phone</label><input type="tel"
                            name="emergency_phone" /></div>
                    <div class="bntm-form-group"><label>Status</label><select name="status">
                    ...
                    <div style="display: flex; gap: 10px; margin-top: 20px;"><button type="submit" class="bntm-btn-primary"
                            id="edit-submit-btn">Update Employee</button><button type="button" id="close-edit-modal"
                            class="bntm-btn-secondary">Cancel</button></div>
                        ...
                        <?php foreach ($employees as $employee):
                            ...
                            if ($tin || $sss || $employee_number || $pagibig) {
                                ...
                                if ($sss)
                                    $tax_info .= 'SSS: ' . $sss . ' ';
                                if ($employee_number)
                                    $tax_info .= 'Emp#: ' . $employee_number . ' ';
                                if ($pagibig)
                                    $tax_info .= 'Pag-IBIG: ' . $pagibig;
                            }
                            ...
                            <tr>
                                ...
                                <td><span
                                        style="padding: 4px 8px; border-radius: 4px; display: inline-block; <?php echo $status_style; ?>"><?php echo esc_html(ucfirst(str_replace('_', ' ', $status))); ?></span>
                                </td>
                                <td><?php echo esc_html($pin ?: 'Not set'); ?></td>
                                <td style="font-size: 9px; white-space: pre-wrap; word-break: break-word;">
                                    <?php echo esc_html($tax_info ?: '-'); ?></td>
                                <td><button class="bntm-btn-small edit-employee"
                                        data-id="<?php echo $employee->ID; ?>">Edit</button><button
                                        class="bntm-btn-small view-attendance" data-id="<?php echo $employee->ID; ?>"
                                        data-name="<?php echo esc_attr($employee->display_name); ?>">Attendance</button><button
                                        class="bntm-btn-small bntm-btn-danger delete-employee"
                                        data-id="<?php echo $employee->ID; ?>"
                                        data-name="<?php echo esc_attr($employee->display_name); ?>">Delete</button></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <?php else: ?>
            <p>No employees found.</p><?php endif; ?>
    </div>
    <script>
        (function () {
            const bntmAjax = { ajax_url: '<?php echo admin_url('admin-ajax.php'); ?>', nonce: '<?php echo wp_create_nonce('bntm_hr_nonce'); ?>' };
...
function bntm_hr_attendance_view($user_id, $can_manage)
{
    ...
    if ($can_manage) {
        if ($filter_employee_id > 0) {
            $attendance = $wpdb->get_results($wpdb->prepare("SELECT a.*, u.display_name FROM {$prefix}hr_attendance a LEFT JOIN {$wpdb->users} u ON a.employee_id = u.ID WHERE a.employee_id = %d AND DATE(a.clock_in) BETWEEN %s AND %s ORDER BY a.clock_in DESC", $filter_employee_id, $date_from, $date_to));
        } else {
            $attendance = $wpdb->get_results($wpdb->prepare("SELECT a.*, u.display_name FROM {$prefix}hr_attendance a LEFT JOIN {$wpdb->users} u ON a.employee_id = u.ID WHERE DATE(a.clock_in) BETWEEN %s AND %s ORDER BY a.clock_in DESC", $date_from, $date_to));
        }
    } else {
        $attendance = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$prefix}hr_attendance WHERE employee_id = %d AND DATE(clock_in) BETWEEN %s AND %s ORDER BY clock_in DESC", $user_id, $date_from, $date_to));
    }
    $all_employees = [];
    if ($can_manage) {
        $all_employees = get_users(['exclude' => [1], 'orderby' => 'display_name']);
    }
    ?>
    <div class="bntm-form-section">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            <h3 style="margin: 0;">Attendance Records<?php if ($employee_name): ?> -
                    <?php echo esc_html($employee_name); ?>    <?php endif; ?></h3><?php if ($filter_employee_id > 0): ?><a
                    href="?type=attendance" class="bntm-btn-secondary">← Back to All Employees</a><?php endif; ?>
        </div>
        <form method="get" class="bntm-form" style="margin-bottom: 20px;"><input type="hidden" name="page"
                value="<?php echo esc_attr($_GET['page'] ?? ''); ?>" /><input type="hidden" name="type"
                value="attendance" />
            <div class="bntm-form-row">
                <?php if ($can_manage): ?>
                    <div class="bntm-form-group"><label>Employee</label><select name="employee_id">
                            <option value="">All Employees</option><?php foreach ($all_employees as $emp): ?>
                                <option value="<?php echo $emp->ID; ?>" <?php selected($filter_employee_id, $emp->ID); ?>>
                                    <?php echo esc_html($emp->display_name); ?></option><?php endforeach; ?>
                        </select></div><?php endif; ?>
                <div class="bntm-form-group"><label>From Date</label><input type="date" name="date_from"
                        value="<?php echo esc_attr($date_from); ?>" /></div>
                <div class="bntm-form-group"><label>To Date</label><input type="date" name="date_to"
                        value="<?php echo esc_attr($date_to); ?>" /></div>
                <div class="bntm-form-group" style="align-self: end;"><button type="submit"
                        class="bntm-btn-primary">Filter</button></div>
            </div>
        </form>
        <?php if ($attendance):
            $total_hours = 0;
            ...
            <div
                ...
                <div style="background: #f0fdf4; padding: 15px; border-radius: 8px; border-left: 4px solid #16a34a;">
                    ...
                    <div style="font-size: 24px; font-weight: bold; color: #0f172a;">
                        <?php echo number_format($total_hours, 2); ?></div>
                </div>
                ...
                <div style="background: #faf5ff; padding: 15px; border-radius: 8px; border-left: 4px solid #9333ea;">
                    ...
                    <div style="font-size: 24px; font-weight: bold; color: #0f172a;">
                        <?php echo $total_days > 0 ? number_format($total_hours / $total_days, 2) : '0'; ?></div>
                </div>
            </div>
            <div class="bntm-table-wrapper">
                <table class="bntm-table">
                    <thead>
                        <tr><?php if ($can_manage && !$filter_employee_id): ?>
                                <th>Employee</th><?php endif; ?>
                            <th>Date</th>
                            <th>Clock In</th>
                            <th>Clock Out</th>
                            <th>Hours</th>
                            <th>Status</th><?php if ($can_manage): ?>
                                <th>Actions</th><?php endif; ?>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($attendance as $record):
                            $status_colors = ['present' => 'background: #d1fae5; color: #065f46;', 'late' => 'background: #fef3c7; color: #92400e;', 'absent' => 'background: #fee2e2; color: #991b1b;', 'on_leave' => 'background: #e0e7ff; color: #3730a3;'];
                            $status_style = isset($status_colors[$record->status]) ? $status_colors[$record->status] : ''; ?>
                            <tr><?php if ($can_manage && !$filter_employee_id): ?>
                                    <td><?php echo esc_html($record->display_name); ?></td><?php endif; ?>
                                <td><?php echo esc_html(date('M d, Y', strtotime($record->clock_in))); ?></td>
                                <td><?php echo esc_html(date('h:i A', strtotime($record->clock_in))); ?></td>
                                <td><?php if ($record->clock_out): ?><?php echo esc_html(date('h:i A', strtotime($record->clock_out))); ?><?php else: ?><span
                                            style="color: #dc2626; font-weight: 500;">Not clocked out</span><?php endif; ?></td>
                                <td><?php if ($record->total_hours): ?><strong><?php echo esc_html(number_format($record->total_hours, 2)); ?>
                                            hrs</strong><?php else: ?><span style="color: #9ca3af;">-</span><?php endif; ?></td>
                                <td><span
                                        style="padding: 4px 8px; border-radius: 4px; display: inline-block; font-size: 12px; <?php echo $status_style; ?>"><?php echo esc_html(ucfirst($record->status)); ?></span>
                                </td>
                                <?php if ($can_manage): ?>
                                    <td><button class="bntm-btn-small edit-attendance" data-id="<?php echo $record->id; ?>"
                                            data-clock-in="<?php echo esc_attr($record->clock_in); ?>"
                                            data-clock-out="<?php echo esc_attr($record->clock_out); ?>"
                                            data-status="<?php echo esc_attr($record->status); ?>">Edit</button></td><?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <div id="edit-attendance-modal"
            ...
            <div style="background: white; padding: 30px; border-radius: 8px; max-width: 500px; width: 90%;">
                ...
                <form id="edit-attendance-form" class="bntm-form"><input type="hidden" name="attendance_id" />
                    <div class="bntm-form-group"><label>Clock In</label><input type="datetime-local" name="clock_in" required />
                    </div>
                    <div class="bntm-form-group"><label>Clock Out</label><input type="datetime-local" name="clock_out" /></div>
                    <div class="bntm-form-group"><label>Status</label><select name="status" required>
                            <option value="present">Present</option>
                            <option value="late">Late</option>
                            <option value="absent">Absent</option>
                            <option value="on_leave">On Leave</option>
                        </select></div>
                    <div style="display: flex; gap: 10px; margin-top: 20px;"><button type="submit"
                            class="bntm-btn-primary">Update</button><button type="button" id="close-attendance-modal"
                            class="bntm-btn-secondary">Cancel</button></div>
...
function bntm_ajax_hr_update_attendance()
{
    ...
    if ($clock_in && $clock_out) {
        ...
        $total_hours = round($interval->h + ($interval->i / 60) + ($interval->days * 24), 2);
    }
    $update_data = ['clock_in' => $clock_in, 'status' => $status];
    $format = ['%s', '%s'];
    if ($clock_out) {
...
function bntm_hr_leaves_view($user_id, $can_manage)
{
    ...
    if (!$can_manage):
        ...
        <div class="bntm-form-section">
            ...
            <form id="leave-request-form" class="bntm-form">
                <div class="bntm-form-group"><label>Leave Type</label><select id="leave-type" required>
                        <option value="">Select type...</option><?php foreach ($leave_types as $type): ?>
                            <option value="<?php echo esc_attr(trim($type)); ?>"><?php echo esc_html(trim($type)); ?></option>
                        <?php endforeach; ?>
                    </select></div>
                <div class="bntm-form-row">
                    <div class="bntm-form-group"><label>Start Date</label><input type="date" id="leave-start-date" required />
                    </div>
                    <div class="bntm-form-group"><label>End Date</label><input type="date" id="leave-end-date" required /></div>
                </div>
    ...
    <?php endif;
    if ($can_manage) {
        $leaves = $wpdb->get_results("SELECT l.*, u.display_name FROM {$prefix}hr_leave_requests l LEFT JOIN {$wpdb->users} u ON l.employee_id = u.ID ORDER BY l.created_at DESC");
    } else {
        $leaves = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$prefix}hr_leave_requests WHERE employee_id = %d ORDER BY created_at DESC", $user_id));
    }
    ?>
    <div class="bntm-form-section">
                ...
                <table class="bntm-table">
                    ...
                    <tbody>
                        <?php foreach ($leaves as $leave):
                            $status_class = $leave->status === 'approved' ? 'bntm-notice-success' : ($leave->status === 'rejected' ? 'bntm-notice-error' : ''); ?>
                            <tr><?php if ($can_manage): ?>
                                    <td><?php echo esc_html($leave->display_name); ?></td><?php endif; ?>
                                <td><?php echo esc_html($leave->leave_type); ?></td>
                                ...
                                <td><?php if ($can_manage && $leave->status === 'pending'): ?><button
                                            class="bntm-btn-small approve-leave"
                                            data-id="<?php echo $leave->id; ?>">Approve</button><button
                                            class="bntm-btn-small bntm-btn-danger reject-leave"
                                            data-id="<?php echo $leave->id; ?>">Reject</button><?php endif; ?></td>
                            </tr>
        ...
        <?php else: ?>
            <p>No leave requests found.</p><?php endif; ?>
    </div>
    <script>
        jQuery(document).ready(function ($) {
            const bntmAjax = { ajax_url: '<?php echo admin_url('admin-ajax.php'); ?>', nonce: '<?php echo wp_create_nonce('bntm_hr_nonce'); ?>' };
...
function bntm_hr_payslips_view($user_id, $can_manage)
{
    ...
    <div class="bntm-form-section">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            ...
            <?php if ($can_manage): ?><button id="generate-payslip-btn" class="bntm-btn-primary">+ Generate
                    Payslip</button><?php endif; ?>
        </div>
        <?php if ($can_manage): ?>
            <!-- Generate Payslip Modal -->
            <div id="payslip-modal"
                style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center;">
                <div
                    style="background: white; padding: 30px; border-radius: 8px; max-width: 600px; width: 90%; max-height: 90vh; overflow-y: auto;">
                    <h3 style="margin-top: 0;">Generate Payslip(s)</h3>
                    <form id="generate-payslip-form" class="bntm-form">
                        <div class="bntm-form-group"><label style="display: flex; align-items: center; gap: 8px;"><input
                                    type="checkbox" id="bulk-generate" style="width:unset" /><span>Generate for multiple
                                    employees</span></label></div>
                        ...
                        <div id="bulk-employee-section" style="display: none;">
                            <div class="bntm-form-group"><label>Select Employees *</label>
                                <div
                                    style="max-height: 200px; overflow-y: auto; border: 1px solid #e5e7eb; border-radius: 6px; padding: 10px;">
                                    <?php foreach ($employees as $emp): ?><label
                                            style="display: block; margin-bottom: 5px;"><input type="checkbox" class="bulk-employee"
                                                value="<?php echo $emp->ID; ?>" /><?php echo esc_html($emp->display_name); ?></label><?php endforeach; ?>
                                </div>
                            </div>
                        </div>
                        <div class="bntm-form-row">
                            <div class="bntm-form-group"><label>Period Start *</label><input type="date" name="period_start"
                                    required /></div>
                            <div class="bntm-form-group"><label>Period End *</label><input type="date" name="period_end"
                                    required /></div>
                        </div>
                        <div class="bntm-form-group"><label
                                style="display: flex; justify-content: space-between; align-items: center;"><span>Manual
                                    Adjustments (Optional)</span><button type="button" id="add-adjustment-btn"
                                    class="bntm-btn-small" style="font-size: 12px; padding: 4px 10px;">+ Add
                                    Adjustment</button></label>
                            <div id="adjustments-container" style="margin-top: 10px;"></div>
                        </div>
                        <div style="display: flex; gap: 10px; margin-top: 20px;"><button type="submit"
                                class="bntm-btn-primary">Generate & Download PDF</button><button type="button"
                                id="close-payslip-modal" class="bntm-btn-secondary">Cancel</button></div>
        ...
        <div style="display: flex; gap: 15px; align-items: center; margin-bottom: 20px; flex-wrap: wrap;">
            <div style="display: flex; gap: 10px; align-items: center;">
                ...
                <select id="month-filter"
                    style="padding: 8px 12px; border: 1px solid #d1d5db; border-radius: 6px; background: white; cursor: pointer;">
            ...
            <?php if ($can_manage && $payslips): ?>
                <button id="bulk-download-payslips" class="bntm-btn-primary" style="padding: 8px 16px; font-size: 14px;">⬇
            ...
            <div class="bntm-table-wrapper">
                <table class="bntm-table">
                    <thead>
                        <tr>
                            <?php if ($can_manage): ?>
                                <th><input type="checkbox" id="select-all-payslips" /></th><?php endif; ?>
                            <?php if ($can_manage): ?>
                                <th>Employee</th><?php endif; ?>
                            <th>Period</th>
                            <th>Basic Pay</th>
                    ...
                    <tbody>
                        <?php foreach ($payslips as $payslip): ?>
                            <tr>
                                <?php if ($can_manage): ?>
                                    <td><input type="checkbox" class="payslip-checkbox" value="<?php echo $payslip->id; ?>"
                                            data-imported="<?php echo $payslip->is_imported; ?>" /></td><?php endif; ?>
                                <?php if ($can_manage): ?>
                                    <td><?php echo esc_html($payslip->display_name); ?></td><?php endif; ?>
                                <td><?php echo date('M d', strtotime($payslip->period_start)) . ' - ' . date('M d, Y', strtotime($payslip->period_end)); ?>
                                </td>
                                ...
                                <td>
                                    <?php if ($payslip->is_imported): ?>
                                        <span
                                            style="padding: 4px 8px; border-radius: 4px; display: inline-block; background: #d1fae5; color: #065f46; font-size: 12px;">✓
                                            Imported</span>
                                    <?php else: ?>
                                        <span
                                            style="padding: 4px 8px; border-radius: 4px; display: inline-block; background: #f3f4f6; color: #6b7280; font-size: 12px;">Not
                                            Imported</span>
                                    <?php endif; ?>
                                </td>
                                <td style="white-space: nowrap;">
                                    <button class="bntm-btn-small download-payslip" data-id="<?php echo $payslip->id; ?>">Download
                                        PDF</button>
                                    <?php if ($can_manage): ?>
...
function bntm_ajax_hr_add_deduction()
{
    ...
    $deductions_array[$key] = ['name' => $name, 'type' => $type, 'value' => $value, 'description' => $description];
...
function bntm_ajax_hr_generate_payslip()
{
    ...
    if ($generated_count > 0)
        wp_send_json_success(['message' => "Generated {$generated_count} payslip(s) successfully!", 'reload' => true]);
    else
...
function bntm_calculate_hours_excluding_lunch($clock_in, $clock_out, $lunch_break_hours = 1)
{
    ...
    $lunch_start = strtotime($work_date . ' 12:00:00');
    $lunch_minutes = ($lunch_break_hours - floor($lunch_break_hours)) * 60;
    $lunch_end = strtotime($work_date . ' ' . floor(12 + $lunch_break_hours) . ':' . intval($lunch_minutes) . ':00');
...
function bntm_ajax_hr_download_payslip()
{
    ...
    if ($token) {
        ...
        if (!$token_data) {
            wp_die('This download link has expired or already been used.');
        }
    ...
    if ($payslip_id === 'latest')
        ...
        $payslip = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$prefix}hr_payslips WHERE id = %d", intval($payslip_id)));
    if (!$payslip)
    ...
    <body>
        <div class="header">
            <div class="header-top">
                <div><?php if ($logo_html): ?><?php echo $logo_html; ?><?php endif; ?>
            ...
            <div class="header-info">
                ...
                <div><strong>Generated:</strong> <?php echo date('M d, Y', strtotime($payslip->created_at ?? 'now')); ?>
                </div>
            </div>
        </div>
        <div class="content">
            <div class="employee-section">
                ...
                if ($tin || $sss || $employee_number || $pagibig):
                    ...
                    <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #ccc;">
                        <div
                            style="font-size: 9pt; font-weight: bold; text-transform: uppercase; margin-bottom: 8px; color: #333;">
                            Tax & Benefits</div>
                        <?php if ($tin): ?>
                            <div class="info-label" style="margin-top: 6px;">TIN</div>
                            <div class="info-value" style="font-size: 9pt;"><?php echo esc_html($tin); ?></div><?php endif; ?>
                        <?php if ($sss): ?>
                            <div class="info-label" style="margin-top: 6px;">SSS</div>
                            <div class="info-value" style="font-size: 9pt;"><?php echo esc_html($sss); ?></div><?php endif; ?>
                        <?php if ($employee_number): ?>
                            <div class="info-label" style="margin-top: 6px;">Emp. Number</div>
                            <div class="info-value" style="font-size: 9pt;"><?php echo esc_html($employee_number); ?></div>
                        <?php endif; ?>
                        <?php if ($pagibig): ?>
                            <div class="info-label" style="margin-top: 6px;">Pag-IBIG</div>
                            <div class="info-value" style="font-size: 9pt;"><?php echo esc_html($pagibig); ?></div><?php endif; ?>
                    </div>
                <?php endif; ?>
            </div>
            <div class="calculation-section">
                        ...
                        <tbody>
                            <?php foreach ($hours_breakdown['breakdown'] as $day): ?>
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <?php echo $day['lunch_deducted'] > 0 ? '-' . number_format($day['lunch_deducted'], 2) : '-'; ?>
                                    </td>
                                    <td class="text-right"><?php echo number_format($day['after_lunch'], 2); ?></td>
                                    <td class="text-right">
                                        <?php echo $day['capped'] > 0 ? '-' . number_format($day['capped'], 2) : '-'; ?></td>
                                    <td class="text-right"><strong><?php echo number_format($day['payable'], 2); ?></strong></td>
                                </tr><?php endforeach; ?>
                            <tr class="subtotal-row">
                                <td colspan="2"><strong>TOTAL</strong></td>
                                <td class="text-right">
                                    <strong><?php echo number_format($hours_breakdown['summary']['total_actual_hours'], 2); ?></strong>
                                </td>
                                <td class="text-right">
                                    <strong>-<?php echo number_format($hours_breakdown['summary']['total_lunch_deducted'], 2); ?></strong>
                                </td>
                                <td class="text-right">
                                    <strong><?php echo number_format($hours_breakdown['summary']['total_actual_hours'] - $hours_breakdown['summary']['total_lunch_deducted'], 2); ?></strong>
                                </td>
                                <td class="text-right">
                                    <strong>-<?php echo number_format($hours_breakdown['summary']['total_capped_hours'], 2); ?></strong>
                                </td>
                                <td class="text-right">
                                    <strong><?php echo number_format($hours_breakdown['summary']['total_payable_hours'], 2); ?></strong>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    <div class="small-text" style="margin-bottom:20px;">Hourly Rate:
                        ₱<?php echo number_format($hourly_rate, 2); ?>/hour | Work Days:
                        <?php echo $hours_breakdown['summary']['work_days']; ?> | Lunch deducted only for shifts &gt; 4 hours
                    </div>
                <?php endif; ?>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($approved_overtime['records'] as $ot):
                                $ot_rate_val = $hourly_rate * $ot_rate_default;
                                $ot_amount = floatval($ot['total_hours']) * $ot_rate_val; ?>
                                <tr>
                            ...
                            <tr class="subtotal-row">
                                ...
                                <td class="text-right">
                                    <strong><?php echo number_format($approved_overtime['summary']['total_hours'], 2); ?></strong>
                                </td>
                                <td class="text-right">
                                    <strong>₱<?php echo number_format($approved_overtime['summary']['total_amount'], 2); ?></strong>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    <div class="small-text" style="margin-bottom:20px;">Overtime Rate: <?php echo $ot_rate_default; ?>x
                        (₱<?php echo number_format($hourly_rate * $ot_rate_default, 2); ?>/hour)</div>
                <?php endif; ?>
                <table>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>Basic Pay (<?php echo number_format($total_hours, 2); ?> hrs ×
                                ₱<?php echo number_format($hourly_rate, 2); ?>)</td>
                        ...
                        <?php if (!empty($deductions_data)): ?>
                            ...
                            <?php foreach ($deductions_data as $deduction): ?>
                                <tr>
                                    ...
                                    <td class="text-right">-₱<?php echo number_format(floatval($deduction['amount'] ?? 0), 2); ?>
                                    </td>
                                </tr><?php endforeach; ?>
                            <tr class="subtotal-row">
                                <td><strong>Total Deductions</strong></td>
                                <td class="text-right"><strong>-₱<?php echo number_format($total_deductions, 2); ?></strong>
                                </td>
                            </tr>
                        <?php endif; ?>
                        <?php if (!empty($adjustments_data)): ?>
                            ...
                            <?php foreach ($adjustments_data as $adjustment):
                                ...
                                <tr>
                                    ...
                                    <td class="text-right">
                                        <?php echo $adjustment['type'] === 'increase' ? '+' : '-'; ?>₱<?php echo number_format($adj_amount, 2); ?>
                                    </td>
                                </tr><?php endforeach; ?>
                        <?php endif; ?>
                        <tr class="total-row">
                ...
                <div class="summary-box">
                    <div class="summary-row"><span>Basic
                            Pay:</span><span>₱<?php echo number_format($basic_pay, 2); ?></span></div>
                    <?php if ($overtime_pay > 0): ?>
                        <div class="summary-row"><span>Overtime
                                Pay:</span><span>₱<?php echo number_format($overtime_pay, 2); ?></span></div><?php endif; ?>
                    <div class="summary-row"><span>Gross
                            Pay:</span><span>₱<?php echo number_format($gross_pay, 2); ?></span></div>
                    <div class="summary-row"><span>Total
                            Deductions:</span><span>-₱<?php echo number_format($total_deductions, 2); ?></span></div>
                    <?php if ($total_adj_increase > 0): ?>
                        <div class="summary-row"><span>Additional
                                Adjustments:</span><span>+₱<?php echo number_format($total_adj_increase, 2); ?></span></div>
                    <?php endif; ?>
                    <?php if ($total_adj_deduction > 0): ?>
                        <div class="summary-row"><span>Adjustment
                                Deductions:</span><span>-₱<?php echo number_format($total_adj_deduction, 2); ?></span></div>
                    <?php endif; ?>
                    <div class="summary-row total"><span>NET
                            PAY:</span><span>₱<?php echo number_format($net_pay, 2); ?></span></div>
                </div>
            </div>
        </div>
        <div class="footer">
            <p>This is a computer-generated payslip. No signature required.</p>
            <p class="small-text">Generated on <?php echo $generated_date; ?> | Document ID: <?php echo $payslip_number; ?>
            </p>
...
function bntm_get_approved_overtime($employee_id, $period_start, $period_end)
{
    ...
    $overtime_records = $wpdb->get_results($wpdb->prepare("SELECT overtime_date as date, start_time, end_time, total_hours FROM {$prefix}hr_overtime WHERE employee_id = %d AND status = 'approved' AND DATE(overtime_date) BETWEEN %s AND %s ORDER BY overtime_date", $employee_id, $period_start, $period_end));
    if (empty($overtime_records))
        return ['records' => [], 'summary' => ['total_hours' => 0, 'total_amount' => 0]];
    $total_hours = 0;
    $total_amount = 0;
    $hourly_rate = floatval(bntm_get_setting('hr_hourly_rate', '15.00'));
    $employee_hourly_rate = floatval(get_user_meta($employee_id, 'bntm_hourly_rate', true)) ?: $hourly_rate;
...
function bntm_ajax_hr_import_payslips()
{
    ...
    foreach ($payslip_ids as $payslip_id) {
        ...
        if ($result) {
            $wpdb->update($prefix . 'hr_payslips', ['is_imported' => 1], ['id' => $payslip->id], ['%d'], ['%d']);
            $imported_count++;
        }
    }
    if ($imported_count > 0) {
...
function bntm_ajax_hr_revert_payslips()
{
    ...
    foreach ($payslip_ids as $payslip_id) {
        ...
        if ($result) {
            $wpdb->update($prefix . 'hr_payslips', ['is_imported' => 0], ['id' => $payslip_id], ['%d'], ['%d']);
            $reverted_count++;
        }
    }
    if ($reverted_count > 0) {
...
function bntm_hr_overtime_missing_view($user_id, $can_manage)
{
    ...
    <?php if (!$can_manage): ?>
        <div class="bntm-form-section" style="margin-bottom: 30px;">
            ...
            <div style="display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid #e5e7eb; padding-bottom: 10px;">
                <button class="overtime-missing-tab active" data-type="overtime"
                    style="background: none; border: none; padding: 10px 15px; cursor: pointer; border-bottom: 2px solid transparent; font-weight: 500; color: var(--bntm-primary, #3b82f6);">Overtime
                    Request</button><button class="overtime-missing-tab" data-type="missing"
                    style="background: none; border: none; padding: 10px 15px; cursor: pointer; border-bottom: 2px solid transparent; font-weight: 500; color: #6b7280;">Missing
                    Clock In/Out</button></div>
            <div id="overtime-form-container" class="overtime-missing-container">
                <form id="overtime-request-form" class="bntm-form">
                    <div class="bntm-form-group"><label>Overtime Date *</label><input type="date" id="overtime-date" required />
                    </div>
                    <div class="bntm-form-row">
                        <div class="bntm-form-group"><label>Start Time *</label><input type="time" id="overtime-start-time"
                                required /></div>
                        <div class="bntm-form-group"><label>End Time *</label><input type="time" id="overtime-end-time"
                                required /></div>
                    </div>
                    <div class="bntm-form-group"><label>Reason for Overtime *</label><textarea id="overtime-reason" rows="3"
            ...
            <div id="missing-form-container" class="overtime-missing-container" style="display: none;">
                <form id="missing-log-form" class="bntm-form">
                    <div class="bntm-form-group"><label>Date of Missing Log *</label><input type="date" id="missing-date"
                            required /></div>
                    <div class="bntm-form-group"><label>Type of Missing Log *</label><select id="missing-type" required>
                    ...
                    <div id="missing-time-fields">
                        <div class="bntm-form-row">
                            <div class="bntm-form-group"><label>Clock In Time</label><input type="time" id="missing-clock-in" />
                            </div>
                            <div class="bntm-form-group"><label>Clock Out Time</label><input type="time"
                                    id="missing-clock-out" /></div>
                        </div>
                    </div>
                    <div class="bntm-form-group"><label>Reason for Missing Log *</label><textarea id="missing-reason" rows="3"
                            required></textarea></div>
    ...
    <div class="bntm-form-section">
        ...
        <?php if ($can_manage) {
            $overtime = $wpdb->get_results("SELECT o.*, u.display_name FROM {$prefix}hr_overtime o LEFT JOIN {$wpdb->users} u ON o.employee_id = u.ID ORDER BY o.created_at DESC");
        } else {
            $overtime = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$prefix}hr_overtime WHERE employee_id = %d ORDER BY created_at DESC", $user_id));
        } ?>
        <?php if ($overtime): ?>
            <div class="bntm-table-wrapper">
                <table class="bntm-table">
                    ...
                    <tbody>
                        <?php foreach ($overtime as $ot):
                            ...
                            $end = DateTime::createFromFormat('H:i:s', $ot->end_time)->format('h:i A'); ?>
                            <tr><?php if ($can_manage): ?>
                                    <td><?php echo esc_html($ot->display_name); ?></td><?php endif; ?>
                                <td><?php echo esc_html(date('M d, Y', strtotime($ot->overtime_date))); ?></td>
                                <td><?php echo esc_html($start . ' - ' . $end); ?></td>
                                <td><?php echo esc_html($ot->total_hours . ' hrs'); ?></td>
                                ...
                                <?php if ($can_manage): ?>
                                    <td><?php if ($ot->status === 'pending'): ?><button class="bntm-btn-small approve-overtime"
                                                data-id="<?php echo $ot->id; ?>">Approve</button><button
                                                class="bntm-btn-small bntm-btn-danger reject-overtime"
                                                data-id="<?php echo $ot->id; ?>">Reject</button><?php endif; ?></td><?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <?php else: ?>
            <p>No overtime requests found.</p><?php endif; ?>
    </div>
    <div class="bntm-form-section">
        <h3><?php echo $can_manage ? 'Missing Clock In/Out Logs' : 'My Missing Logs'; ?></h3>
        <?php if ($can_manage) {
            $missing = $wpdb->get_results("SELECT m.*, u.display_name FROM {$prefix}hr_missing_logs m LEFT JOIN {$wpdb->users} u ON m.employee_id = u.ID ORDER BY m.created_at DESC");
        } else {
            $missing = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$prefix}hr_missing_logs WHERE employee_id = %d ORDER BY created_at DESC", $user_id));
        } ?>
        <?php if ($missing): ?>
            <div class="bntm-table-wrapper">
                <table class="bntm-table">
                    ...
                    <tbody>
                        <?php foreach ($missing as $log):
                            ...
                            $clock_out = $log->clock_out_time ? DateTime::createFromFormat('H:i:s', $log->clock_out_time)->format('h:i A') : '-'; ?>
                            <tr><?php if ($can_manage): ?>
                                    <td><?php echo esc_html($log->display_name); ?></td><?php endif; ?>
                                <td><?php echo esc_html(date('M d, Y', strtotime($log->log_date))); ?></td>
                                <td><?php echo esc_html($type_label); ?></td>
                                <td><?php echo esc_html($clock_in); ?></td>
                                <td><?php echo esc_html($clock_out); ?></td>
                                ...
                                <?php if ($can_manage): ?>
                                    <td><?php if ($log->status === 'pending'): ?><button class="bntm-btn-small approve-missing"
                                                data-id="<?php echo $log->id; ?>">Approve</button><button
                                                class="bntm-btn-small bntm-btn-danger reject-missing"
                                                data-id="<?php echo $log->id; ?>">Reject</button><?php endif; ?></td><?php endif; ?>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
        ...
        <?php else: ?>
            <p>No missing logs found.</p><?php endif; ?>
    </div>
