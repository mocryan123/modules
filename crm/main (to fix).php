                    ...
                    <?php foreach ($schedules as $schedule): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                if ($payment_plan && isset($payment_plan['type'])) {
                                    switch($payment_plan['type']) {
                                        ...
                                        case 'full':
                                            ...
                                            break;
                                        case 'percentage':
                                            echo "Custom %: " . $payment_plan['percentages'];
                                            break;
                                    }
                                }
                                ?>
            ...
            <form id="schedule-form" class="bntm-form">
                ...
                <div id="full-plan-fields" style="display: none;">
                    <div class="bntm-form-group">
                        ...
                        <small>Each invoice will be for the full quotation amount</small>
                    </div>
                    <div style="background: #fef3c7; padding: 12px; border-radius: 4px; margin-top: 10px;">
                        <strong>⚠️ Note:</strong> Each invoice will bill the full quotation amount. This is useful for recurring services or subscriptions.
                    </div>
                </div>
...
function bntm_ajax_crm_create_billing_schedule() {
    ...
    $payment_plan_json = sanitize_textarea_field($_POST['payment_plan_json'] ?? '');
    $start_date = sanitize_text_field($_POST['start_date'] ?? '');
    $end_date = sanitize_text_field($_POST['end_date'] ?? '');
    $notes = sanitize_textarea_field($_POST['notes'] ?? '');
    
    if ($quotation_id <= 0 || $customer_id <= 0 || empty($start_date) || empty($payment_plan_json)) {
        wp_send_json_error(['message' => 'Required fields are missing']);
    }
    
    $payment_plan = json_decode($payment_plan_json, true);
    
    // Determine totalinvoices and amount based on plan type
    $total_invoices = 0;
    $amount_per_invoice = 0;
    if ($payment_plan['type'] === 'divided') {
        $total_invoices = $payment_plan['installments'];
        $amount_per_invoice = $payment_plan['amount_per_invoice'];
    } elseif ($payment_plan['type'] === 'full') {
        $total_invoices = $payment_plan['total_invoices'];
        $amount_per_invoice = $payment_plan['amount_per_invoice'];
    ...
    $result = $wpdb->insert($schedules_table, [
        ...
        'payment_plan' => $payment_plan_json,
...
function crm_generate_invoice_from_schedule($schedule_id) {
    ...
    if ($payment_plan['type'] === 'divided') {
        $invoice_amount = $payment_plan['amount_per_invoice'];
    } elseif ($payment_plan['type'] === 'full') {
        $invoice_amount = $payment_plan['amount_per_invoice'];
    } elseif ($payment_plan['type'] === 'percentage') {
        $percentage = $payment_plan['percentage_array'][$current_invoice_number - 1];
        $invoice_amount = $payment_plan['amounts'][$current_invoice_number - 1];
        $percentage_info = " ({$percentage}% of total)";
    }
    
    // Build description from quotation items with proper line breaks
    $description_lines = [];
    ...
    if (!empty($quotation_items)) {
        foreach ($quotation_items as $item) {
            ...
            // Add item description if exists
    ...
    $description_lines[] = "Invoice " . $current_invoice_number;
    if ($schedule->total_invoices > 0) {
        $description_lines[] = "of " . $schedule->total_invoices . $percentage_info;
    }
    ...
    if ($result) {
        ...
        if (!empty($quotation_items)) {
            foreach ($quotation_items as $item) {
                ...
                $item_proportion = $item->total / $quotation->total;
                $item_invoice_amount = $invoice_amount * $item_proportion;
