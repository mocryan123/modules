...
function ps_render_js() {
    ...
    window.psGenerateDailySummaryFromOverview = function() {
        ...
        document.getElementById('print-modal-subtitle').textContent = 'Date: ' + dt + ' - Review before printing';
...
function bntm_ajax_ps_save_settings() {
    ...
    if (isset($_POST['ps_doc_custom_css'])) {
        bntm_save_setting('ps_doc_custom_css', sanitize_textarea_field($_POST['ps_doc_custom_css']));
    }
    ...
    foreach (array_keys(ps_get_document_definitions()) as $doc_key) {
        ...
        foreach ($doc_text_fields as $field) {
            if (isset($_POST[$field])) {
                bntm_save_setting($field, sanitize_text_field($_POST[$field]));
            }
        }

        $css_field = $prefix . 'custom_css';
        if (isset($_POST[$css_field])) {
            bntm_save_setting($css_field, sanitize_textarea_field($_POST[$css_field]));
        }
    }
...
function bntm_ajax_ps_renew_loan() {
    ...
    $wpdb->insert($wpdb->prefix.'ps_payments', [
        ...
        'payment_method'=>$payment_method,'reference_number'=>$reference_number,'processed_by'=>$business_id,'notes'=>$payment_notes,
    ]);
...
function bntm_ajax_ps_redeem_loan() {
    ...
    $wpdb->insert($wpdb->prefix.'ps_payments', [
        ...
        'reference_number'=>$reference_number,'processed_by'=>$business_id,'notes'=>$notes,'status'=>'completed',
    ]);
