...
function bntm_pm_get_tables()
{
    ...
    return [
        ...
        "pm_active_task_sessions" => "CREATE TABLE {$prefix}pm_active_task_sessions (
            id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
            rand_id VARCHAR(20) UNIQUE NOT NULL,
            business_id BIGINT UNSIGNED NOT NULL DEFAULT 0,
            task_id BIGINT UNSIGNED NOT NULL,
...
function pm_ensure_active_session_table()
{
    ...
    $charset = $wpdb->get_charset_collate();
    $table = $wpdb->prefix . "pm_active_task_sessions";
...
function pm_recalculate_task_actual_hours($task_id)
{
    global $wpdb;
...
function pm_get_utc_mysql_now()
{
    return gmdate("Y-m-d H:i:s");
}

function pm_parse_client_datetime_to_utc_mysql($client_datetime)
{
    if (empty($client_datetime)) {
        return pm_get_utc_mysql_now();
    }

    try {
        $dt = new DateTime($client_datetime);
...
function pm_utc_mysql_to_iso8601($datetime)
{
    if (empty($datetime)) {
        return "";
    }
...
function pm_create_automatic_time_log($session, $notes = "")
{
    global $wpdb;
    ...
    $started_ts = strtotime(pm_utc_mysql_to_iso8601($session->started_at));
    $ended_ts = current_time("timestamp", true);

    if (!$started_ts || $ended_ts <= $started_ts) {
        return ["success" => false, "message" => "Invalid timer duration"];
    }

    $hours = max(0.01, round(($ended_ts - $started_ts) / 3600, 2));

    if ($hours <= 0) {
        return ["success" => false, "message" => "No elapsed time to log"];
    }
    ...
    $inserted = $wpdb->insert(
        ...
        [
            ...
            "hours" => $hours,
            "log_date" => wp_date("Y-m-d", $ended_ts, wp_timezone()),
            "notes" => $notes,
        ],
        ["%s", "%d", "%d", "%d", "%f", "%s", "%s"]
    );
    ...
    if (!$inserted) {
        return ["success" => false, "message" => "Failed to save automatic time log"];
    }

    pm_recalculate_task_actual_hours($session->task_id);

    return [
        "success" => true,
        "task" => $task,
        "hours" => $hours,
        "started_at" => pm_utc_mysql_to_iso8601($session->started_at),
        "ended_at" => gmdate("c", $ended_ts),
    ];
}

function pm_get_current_user_active_session()
{
    pm_ensure_active_session_table();

    global $wpdb;
    $sessions_table = $wpdb->prefix . "pm_active_task_sessions";
...
function pm_render_task_timer_button($task_id, $is_active, $context = "default")
{
    $label = $is_active ? "Active" : "Inactive";
    $classes = "pm-task-timer-btn" . ($is_active ? " is-active" : "");

    if ($context === "row") {
        $classes .= " pm-task-timer-btn-row";
    } elseif ($context === "board") {
        $classes .= " pm-task-timer-btn-board";
    } elseif ($context === "modal") {
        $classes .= " pm-task-timer-btn-modal";
    }

    ob_start();
    ?>
    <button type="button" class="<?php echo esc_attr($classes); ?>" data-task-id="<?php echo (int) $task_id; ?>"
        data-active="<?php echo $is_active ? "1" : "0"; ?>">
        <span class="pm-task-timer-btn-dot"></span>
        <span class="pm-task-timer-btn-label"><?php echo esc_html($label); ?></span>
    </button>
    <?php
    return ob_get_clean();
}
...
add_action("wp_ajax_pm_start_task_timer", "bntm_ajax_pm_start_task_timer");
add_action("wp_ajax_pm_stop_task_timer", "bntm_ajax_pm_stop_task_timer");
            ...
            if ($active_tab === "overview") {
                ...
                if ($can_access_resources) {
                    ...
                    <div style="background: white; border-radius: 12px; padding: 60px 24px; text-align: center;">
                        ...
                        <p style="margin: 0; color: #6b7280; font-size: 15px;">You don't have permission to view resource
                            management. This feature is only available to administrators, owners, and managers.</p>
                    </div>
                    <?php
                }
                            ...
                            foreach ($status_stats as $stat):
                                ...
                                <div class="chart-bar-item">
                                    ...
                                    <div class="chart-bar-wrapper">
                                        <div class="chart-bar"
                                            style="width: <?php echo $percentage; ?>%; background: <?php echo $color; ?>"></div>
                            ...
                            <div class="project-info">
                                ...
                                <div class="project-meta">
                                    <span class="project-status"
                                        style="background: <?php echo pm_get_status_color($project->status); ?>">
                                    ...
                                    <?php endif; ?>
                                    <span class="project-tasks"><?php echo $completed_count; ?>/<?php echo $tasks_count; ?>
                                        tasks</span>
                                </div>
                                <div class="project-progress">
                                    <div class="progress-bar">
                                        <div class="progress-fill"
                                            style="width: <?php echo $progress; ?>%; background: <?php echo esc_attr($project->color); ?>">
                                        </div>
                                    </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($projects as $project):
                            ...
                            $progress =
                                ...
                                : 0;
                            ?>
                            <tr class="pm-project-row" onclick="window.location.href='?project_id=<?php echo $project->id; ?>'"
                                ...
                                <td onclick="event.stopPropagation();">
                                    <?php if ($can_manage_project): ?>
                                        <div style="display: flex; gap: 8px;">
                                            <button class="bntm-btn-small bntm-btn-secondary" onclick="pmEditProject(<?php echo htmlspecialchars(
                                                json_encode($project)
                                            ); ?>)" title="Edit">
                                                <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <button class="bntm-btn-small bntm-btn-danger" onclick="pmDeleteProject(<?php echo $project->id; ?>, '<?php echo esc_js(
                                                   $project->name
                                               ); ?>')" title="Delete">
                                                <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
function pm_board_tab($business_id)
{
    ...
    $current_session = pm_get_current_user_active_session();
    $active_task_id = $current_session ? (int) $current_session->task_id : 0;
    $is_wp_admin = current_user_can('manage_options');
    $current_role = bntm_get_user_role($current_user_id);
    $can_view_all = $is_wp_admin || in_array($current_role, ['owner', 'manager'], true);

    $detail_nonce = wp_create_nonce('pm_project_detail_nonce');
    $reorder_nonce = wp_create_nonce('pm_reorder_nonce');
    ...
    if ($can_view_all) {
        ...
        $projects = $wpdb->get_results($wpdb->prepare(
            "SELECT p.* FROM {$projects_table} p
             INNER JOIN {$team_table} tm ON tm.project_id = p.id AND tm.user_id = %d
    ...
    if (empty($projects)) {
        return '<div class="pm-board-container"><div class="pm-empty-board">
            <h3>No Active Projects</h3><p>You are not a member of any active project yet.</p>
            <a href="?tab=projects" class="bntm-btn-primary">Go to Projects</a>
            </div></div>';
    }

    $project_ids = array_map(fn($p) => (int) $p->id, $projects);
    $id_ph = implode(',', array_fill(0, count($project_ids), '%d'));
    $project_map = [];
    foreach ($projects as $p)
        $project_map[(int) $p->id] = $p;

    /* ── Milestones ─────────────────────────────────────────────────────── */
    $all_milestones_raw = $wpdb->get_results($wpdb->prepare(
        "SELECT * FROM {$milestones_table} WHERE project_id IN ({$id_ph})",
        ...$project_ids
    ));
    $milestones_by_project = [];
    $milestone_map = [];
    foreach ($all_milestones_raw as $m) {
        $milestones_by_project[(int) $m->project_id][] = $m;
        $milestone_map[(int) $m->id] = $m;
    }

    /* ── Custom statuses ────────────────────────────────────────────────── */
    $all_statuses_raw = $wpdb->get_results($wpdb->prepare(
        "SELECT * FROM {$statuses_table} WHERE project_id IN ({$id_ph}) ORDER BY project_id, sort_order",
        ...$project_ids
    ));
    $statuses_by_project = [];
    foreach ($all_statuses_raw as $s) {
        $statuses_by_project[(int) $s->project_id][] = $s;
    }

    $project_statuses_js = [];
    foreach ($statuses_by_project as $pid => $statuses) {
        $project_statuses_js[$pid] = array_map(fn($s) => $s->status_name, $statuses);
    }

    /* ── Team members ───────────────────────────────────────────────────── */
    $all_team_raw = $wpdb->get_results($wpdb->prepare(
        "SELECT tm.*, u.display_name FROM {$team_table} tm
         INNER JOIN {$wpdb->users} u ON tm.user_id = u.ID
         WHERE tm.project_id IN ({$id_ph})",
        ...$project_ids
    ));
    $team_by_project = [];
    foreach ($all_team_raw as $tm) {
        $team_by_project[(int) $tm->project_id][] = $tm;
    }

    /* ── Tasks (exclude closed) ─────────────────────────────────────────── */
    if ($can_view_all) {
        $tasks = $wpdb->get_results($wpdb->prepare(
            "SELECT t.*, p.name AS project_name, p.color AS project_color,
             ...
             WHERE t.project_id IN ({$id_ph}) AND t.status NOT IN ('closed')
        ...
        $tasks = $wpdb->get_results($wpdb->prepare(
            "SELECT t.*, p.name AS project_name, p.color AS project_color,
             ...
             INNER JOIN {$projects_table} p ON t.project_id = p.id
             LEFT  JOIN {$wpdb->users}   u ON t.assigned_to = u.ID
             WHERE t.project_id IN ({$id_ph}) AND t.assigned_to = %d AND t.status NOT IN ('closed')
             ORDER BY t.sort_order ASC, t.created_at DESC",
            ...array_merge($project_ids, [$current_user_id])
        ));
    }

    if (empty($tasks)) {
        return '<div class="pm-board-container"><div class="pm-empty-board">
            <h3>No Tasks Found</h3><p>No active tasks available.</p></div></div>';
    }

    /* ── Bulk-fetch logs & comments ─────────────────────────────────────── */
    $task_ids = array_map(fn($t) => (int) $t->id, $tasks);
    $t_ph = implode(',', array_fill(0, count($task_ids), '%d'));

    $hours_raw = $wpdb->get_results($wpdb->prepare(
        "SELECT task_id, SUM(hours) AS total_hours FROM {$time_logs_table}
         WHERE task_id IN ({$t_ph}) GROUP BY task_id",
        ...$task_ids
    ));
    $hours_map = [];
    foreach ($hours_raw as $h)
        $hours_map[(int) $h->task_id] = floatval($h->total_hours);

    $latest_comments_raw = $wpdb->get_results($wpdb->prepare(
        "SELECT c.task_id, c.comment, u.display_name, c.created_at
         FROM {$comments_table} c LEFT JOIN {$wpdb->users} u ON c.user_id = u.ID
         WHERE c.task_id IN ({$t_ph})
           AND c.id IN (SELECT MAX(id) FROM {$comments_table} WHERE task_id IN ({$t_ph}) GROUP BY task_id)",
        ...array_merge($task_ids, $task_ids)
    ));
    $comment_map = [];
    foreach ($latest_comments_raw as $c)
        $comment_map[(int) $c->task_id] = $c;

    $latest_logs_raw = $wpdb->get_results($wpdb->prepare(
        "SELECT tl.task_id, tl.hours, tl.log_date, tl.notes, u.display_name
         FROM {$time_logs_table} tl LEFT JOIN {$wpdb->users} u ON tl.user_id = u.ID
         WHERE tl.task_id IN ({$t_ph})
           AND tl.id IN (SELECT MAX(id) FROM {$time_logs_table} WHERE task_id IN ({$t_ph}) GROUP BY task_id)",
        ...array_merge($task_ids, $task_ids)
    ));
    $log_map = [];
    foreach ($latest_logs_raw as $l)
        $log_map[(int) $l->task_id] = $l;

    ob_start();
    ?>
    <script>
        var ajaxurl = '<?php echo esc_js(admin_url('admin-ajax.php')); ?>';
        var pmBoardDetailNonce = '<?php echo esc_js($detail_nonce); ?>';
        var pmBoardReorderNonce = '<?php echo esc_js($reorder_nonce); ?>';
        var pmBoardCurrentUserId = <?php echo $current_user_id; ?>;
        var pmBoardActiveProjectId = 0;
        var pmBoardProjectStatuses = <?php echo json_encode($project_statuses_js); ?>;
    </script>
    ...
    <div class="pm-board-container">
        ...
        <div class="pm-board-header">
            <div class="pm-board-title-group">
                <h2>Project Board</h2>
                <span id="pmb-visible-count-badge"><?php echo count($tasks); ?></span>
                <span class="pmb-count-label">tasks</span>
            </div>
            <div class="pm-board-controls">
                <div class="pm-board-filter-toggle">
                    <button class="pm-board-filter-btn active" data-filter="all" onclick="pmbSetAssignment('all')">All
                        Tasks</button>
                    <button class="pm-board-filter-btn" data-filter="my" onclick="pmbSetAssignment('my')">My Tasks</button>
                </div>

                <select id="pmb-project-filter" class="bntm-input pmb-ctrl pmb-dropdown" onchange="pmbOnProjectChange()">
                ...
                <select id="pmb-status-filter" class="bntm-input pmb-ctrl pmb-dropdown" onchange="pmbApplyFilters()"
                    style="display:none;">
                    <option value="">All Statuses</option>
                </select>

                <select id="pmb-deadline-sort" class="bntm-input pmb-ctrl pmb-dropdown" onchange="pmbApplyFilters()">
                ...
                <div class="pmb-search-wrap">
                    <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                        class="pmb-search-icon">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg>
                    <input type="text" id="pmb-search" placeholder="Search tasks…"
                        class="bntm-input pmb-ctrl pmb-search-input" oninput="pmbApplyFilters()">
                </div>
            </div>
        </div>

        <div class="pmb-table-wrap">
            <table class="pmb-table" id="pmb-task-table">
                <thead>
                    <tr>
                        <!-- Col 1: drag handle -->
                        <th class="pmb-col-drag"></th>
                        <!-- Col 2: priority bar -->
                        <th class="pmb-col-pri"></th>
                        <!-- Col 3: Task title (widest) -->
                        <th class="pmb-col-title">Task</th>
                        <!-- Col 4: Status (inline change) -->
                        <th class="pmb-col-status">Status</th>
                        <!-- Col 5: Assignee -->
                        <th class="pmb-col-assigned">Assigned</th>
                        <!-- Col 6: Due date (sortable) -->
                        <th class="pmb-col-due pmb-sortable" onclick="pmbToggleSort('due')">
                            Due <span class="pmb-sort-icon" id="pmb-due-icon">↕</span>
                        </th>
                        <!-- Col 7: Project -->
                        <th class="pmb-col-project">Project</th>
                        <!-- Col 8: Milestone -->
                        <th class="pmb-col-milestone">Milestone</th>
                        <!-- Col 9: Hours -->
                        <th class="pmb-col-hours">Hours</th>
                        <!-- Col 10: Latest comment -->
                        <th class="pmb-col-comment">Comment</th>
                        <!-- Col 11: Latest log -->
                        <th class="pmb-col-log">Log</th>
                        <!-- Col 12: Quick Actions (timer + edit + delete) -->
                        <th class="pmb-col-actions">Actions</th>
                    </tr>
                </thead>
                <tbody id="pmb-tbody">
                    <?php foreach ($tasks as $task):
                        $tid = (int) $task->id;
                        $proj = $project_map[(int) $task->project_id] ?? null;
                        $milestone = $task->milestone_id ? ($milestone_map[(int) $task->milestone_id] ?? null) : null;
                        $is_overdue = $task->due_date && strtotime($task->due_date) < time() && $task->status !== 'completed';
                        $due_ts = $task->due_date ? strtotime($task->due_date) : 0;
                        $hours_logg = $hours_map[$tid] ?? 0;
                        $lat_comment = $comment_map[$tid] ?? null;
                        $lat_log = $log_map[$tid] ?? null;
                        $status_col = pm_get_status_color($task->status);
                        $is_task_active = $active_task_id === $tid;

                        $proj_statuses = $statuses_by_project[(int) $task->project_id] ?? [];

                        /* Check if any status in this project is named 'closed' (case-insensitive).
                           If not, append a synthetic 'Closed' entry so users can always close a task. */
                        $has_closed = false;
                        foreach ($proj_statuses as $s) {
                            if (strtolower(trim($s->status_name)) === 'closed') {
                                $has_closed = true;
                                break;
                            }
                        }
                        ...
                        $status_opts = '';
                        foreach ($proj_statuses as $s) {
                            $sc = esc_attr(pm_get_status_color($s->status_name));
                            $sn = esc_attr($s->status_name);
                            $status_opts .= '<div class="task-status-option pm-task-status-option"'
                                . ' data-task-id="' . $tid . '" data-status="' . $sn . '" data-color="' . $sc . '"'
                                . ' style="background:' . $sc . ';">' . esc_html($s->status_name) . '</div>';
                        }

                        /* Append 'Closed' only when it is missing from this project's statuses */
                        if (!$has_closed) {
                            $closed_color = esc_attr(pm_get_status_color('closed'));
                            $status_opts .= '<div class="task-status-option pm-task-status-option pmb-status-closed-synthetic"'
                                . ' data-task-id="' . $tid . '" data-status="closed" data-color="' . $closed_color . '"'
                                . ' style="background:' . $closed_color . ';">Closed</div>';
                        }

                        $proj_team_arr = array_map(fn($m) => ['user_id' => $m->user_id, 'display_name' => $m->display_name], $team_by_project[(int) $task->project_id] ?? []);
                        $proj_ms_arr = array_map(fn($m) => ['id' => $m->id, 'name' => $m->name], $milestones_by_project[(int) $task->project_id] ?? []);
                        $proj_stat_arr = array_map(fn($s) => ['status_name' => $s->status_name], $proj_statuses);

                        /* Mirror the same 'Closed' fallback into the modal status dropdown data */
                        if (!$has_closed) {
                            $proj_stat_arr[] = ['status_name' => 'Closed'];
                        }

                        $comment_tooltip = '';
                        if ($lat_comment) {
                            $comment_tooltip = esc_attr(json_encode([
                                'author' => $lat_comment->display_name,
                                'text' => $lat_comment->comment,
                                'created' => $lat_comment->created_at,
                            ]));
                        }

                        $log_tooltip = '';
                        if ($lat_log) {
                            $log_tooltip = esc_attr(json_encode([
                                'author' => $lat_log->display_name,
                                'hours' => $lat_log->hours,
                                'date' => $lat_log->log_date,
                                'notes' => $lat_log->notes,
                            ]));
                        }
                        ?>
                        <tr class="pmb-row <?php echo $is_task_active ? 'pmb-row--active' : ''; ?>" draggable="true"
                            data-task-id="<?php echo $tid; ?>" data-project-id="<?php echo (int) $task->project_id; ?>"
                            data-assigned-to="<?php echo (int) ($task->assigned_to ?? 0); ?>"
                            data-task-status="<?php echo esc_attr($task->status); ?>" data-due-ts="<?php echo $due_ts; ?>"
                            data-is-active="<?php echo $is_task_active ? '1' : '0'; ?>"
                            data-milestones="<?php echo esc_attr(json_encode($proj_ms_arr)); ?>"
                            data-team="<?php echo esc_attr(json_encode($proj_team_arr)); ?>"
                            data-statuses="<?php echo esc_attr(json_encode($proj_stat_arr)); ?>">

                            <!-- drag -->
                            <td class="pmb-drag-cell pmb-col-drag">
                                <svg width="12" height="14" fill="currentColor" viewBox="0 0 12 14">
                                    ...
                                    <circle cx="8.5" cy="11.5" r="1.2" />
                                </svg>
                            </td>

                            <!-- priority bar -->
                            <td class="pmb-priority-cell pmb-col-pri">
                                <span class="pmb-priority-bar"
                                    style="background:<?php echo esc_attr($proj->color ?? '#6b7280'); ?>;"
                                    title="Priority: <?php echo esc_attr($task->priority); ?>"></span>
                            </td>

                            <!-- title + tags -->
                            <td class="pmb-title-cell pmb-col-title">
                                <div class="pmb-title-inner">
                                    <span class="pmb-prio-dot priority-<?php echo esc_attr($task->priority); ?>"></span>
                                    <span class="pmb-task-title"><?php echo esc_html($task->title); ?></span>
                                </div>
                                <?php if ($task->tags): ?>
                                    <div class="pmb-tags">
                            ...
                            <td class="pmb-status-cell pmb-col-status">
                                <div class="task-status-dropdown pmb-dropdown">
                                    <button type="button" class="task-status-btn pm-task-status-btn"
                                        style="background:<?php echo esc_attr($status_col); ?>;">
                                    ...
                                    <div class="task-status-menu"><?php echo $status_opts; ?></div>
                                </div>
                            </td>

                            <!-- assigned -->
                            <td class="pmb-col-assigned">
                                <?php if ($task->assigned_name): ?>
                                    <span class="pmb-assignee">
                                        <span
                                            class="pmb-avatar"><?php echo esc_html(mb_substr($task->assigned_name, 0, 1)); ?></span>
                                        <span class="pmb-assignee-name"><?php echo esc_html($task->assigned_name); ?></span>
                                    </span>
                                <?php else: ?><span class="pmb-null">Unassigned</span><?php endif; ?>
                            </td>

                            <!-- due date -->
                            <td class="pmb-col-due">
                                <?php if ($task->due_date): ?>
                                    <div class="pmb-due <?php echo $is_overdue ? 'overdue' : ''; ?>">
                                        <?php echo date('M d, Y', $due_ts); ?>
                                        <?php if ($is_overdue): ?><span class="pmb-overdue-chip">Overdue</span><?php endif; ?>
                                    </div>
                                <?php else: ?><span class="pmb-null">—</span><?php endif; ?>
                            </td>

                            <!-- project -->
                            <td class="pmb-col-project">
                                <?php if ($proj): ?>
                                    <a href="?project_id=<?php echo (int) $proj->id; ?>&subtab=overview" class="pmb-project-link"
                                        style="--pc:<?php echo esc_attr($proj->color); ?>;">
                                        <span class="pmb-proj-color-dot"
                                            style="background:<?php echo esc_attr($proj->color); ?>;"></span>
                            ...
                            <!-- milestone -->
                            <td class="pmb-col-milestone">
                                <?php if ($milestone): ?>
                                    <span class="pmb-ms-badge">
                            ...
                            <!-- hours logged -->
                            <td class="pmb-col-hours">
                                <span class="pmb-hours <?php echo $hours_logg > 0 ? 'has-hours' : ''; ?>">
                                    <?php echo number_format($hours_logg, 1); ?>h
                                </span>
                            </td>

                            <!-- latest comment -->
                            <td class="pmb-snippet-cell pmb-col-comment">
                                <?php if ($lat_comment): ?>
                                    <div class="pmb-snip-wrap pmb-snip-comment" data-tooltip="<?php echo $comment_tooltip; ?>"
                                        tabindex="0" role="button" aria-label="Quick preview comment">
                                        <div class="pmb-snip-row">
                                            <span class="pmb-snip-icon">
                                                <svg width="11" height="11" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M8 10h.01M12 10h.01M16 10h.01M21 12c0 4.418-4.03 8-9 8a9.77 9.77 0 01-4-.844L3 20l1.07-3.685C3.396 15.01 3 13.55 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                                                </svg>
                                            </span>
                                            <span class="pmb-snip-author"><?php echo esc_html($lat_comment->display_name); ?></span>
                                        </div>
                                        <span
                                            class="pmb-snip-text"><?php echo esc_html(wp_trim_words($lat_comment->comment, 6)); ?></span>
                                    </div>
                                <?php else: ?><span class="pmb-null">—</span><?php endif; ?>
                            </td>

                            <!-- latest log -->
                            <td class="pmb-snippet-cell pmb-col-log">
                                <?php if ($lat_log): ?>
                                    <div class="pmb-snip-wrap pmb-snip-log" data-tooltip="<?php echo $log_tooltip; ?>" tabindex="0"
                                        role="button" aria-label="Quick preview time log">
                                        <div class="pmb-snip-row">
                                            <span class="pmb-snip-icon">
                                                <svg width="11" height="11" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <span class="pmb-snip-author"><?php echo esc_html($lat_log->display_name); ?></span>
                                        </div>
                                        <span class="pmb-snip-text"><?php echo number_format($lat_log->hours, 1); ?>h ·
                                            <?php echo esc_html(date('M d', strtotime($lat_log->log_date))); ?></span>
                                    </div>
                                <?php else: ?><span class="pmb-null">—</span><?php endif; ?>
                            </td>

                            <!-- quick actions: timer | edit | delete -->
                            <td class="pmb-actions-cell pmb-col-actions">
                                <?php echo pm_render_task_timer_button($tid, $is_task_active, 'board'); ?>
                                <button class="pmb-action-btn pmb-action-edit pm-edit-task-btn"
                                    data-task-id="<?php echo $tid; ?>" title="Edit task">
                                    <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button class="pmb-action-btn pmb-action-delete pm-delete-task-btn"
                                    data-task-id="<?php echo $tid; ?>" data-task-title="<?php echo esc_attr($task->title); ?>"
                                    title="Delete task">
                                    <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
            ...
            <div id="pmb-no-results" style="display:none;text-align:center;padding:48px;color:#9ca3af;font-size:14px;">
    ...
    <!-- Tooltip — single instance, repositioned by JS. No internal scroll. -->
    <div id="pmb-snip-tooltip" class="pmb-snip-tooltip" role="tooltip" aria-live="polite">
        <div class="pmb-snip-tooltip-header">
            <span class="pmb-snip-tooltip-author"></span>
            <span class="pmb-snip-tooltip-meta"></span>
        </div>
        <div class="pmb-snip-tooltip-body"></div>
    </div>

    <!-- Task Modal -->
    <div id="pm-board-task-modal" class="bntm-modal" style="display:none;">
        <div class="bntm-modal-content" style="max-width:700px;">
            <div class="bntm-modal-header">
                <h3 id="pmb-modal-title">Edit Task</h3>
                <button class="bntm-modal-close" onclick="pmbCloseTaskModal()">&times;</button>
            </div>
            <form id="pm-board-task-form" class="bntm-form">
                <input type="hidden" name="task_id" id="pmb-task-id">
                <input type="hidden" name="project_id" id="pmb-task-project-id">
                <div class="bntm-form-group">
                    <label>Task Title *</label>
                    <input type="text" name="title" id="pmb-task-title" required class="bntm-input">
                </div>
                <div class="bntm-form-group">
                    <label>Description</label>
                    <textarea name="description" id="pmb-task-desc" rows="4" class="bntm-input"></textarea>
                </div>
                <div class="bntm-form-row">
                    <div class="bntm-form-group">
                        <label>Status</label>
                        <select name="status" id="pmb-task-status" class="bntm-input pmb-dropdown"></select>
                    </div>
                    <div class="bntm-form-group">
                        <label>Priority</label>
                        <select name="priority" id="pmb-task-priority" class="bntm-input pmb-dropdown">
                ...
                <div class="bntm-form-row">
                    <div class="bntm-form-group">
                        ...
                        <select name="assigned_to" id="pmb-task-assigned" class="bntm-input pmb-dropdown"></select>
                    </div>
                    <div class="bntm-form-group">
                        <label>Milestone</label>
                        <select name="milestone_id" id="pmb-task-milestone" class="bntm-input pmb-dropdown"></select>
                    </div>
                </div>
                <div class="bntm-form-row">
                    <div class="bntm-form-group">
                        <label>Due Date</label>
                        <input type="date" name="due_date" id="pmb-task-due" class="bntm-input">
                    </div>
                    <div class="bntm-form-group">
                        <label>Estimated Hours</label>
                        <input type="number" name="estimated_hours" id="pmb-task-estimated" step="0.1" min="0"
                            class="bntm-input">
                    </div>
                </div>
                <div class="bntm-form-group">
                    <label>Tags (comma separated)</label>
                    <input type="text" name="tags" id="pmb-task-tags" placeholder="design, frontend, urgent"
                        class="bntm-input">
                </div>

                <div id="pmb-task-logs-section" style="display:none;">
                    <hr style="margin:24px 0;border:none;border-top:1px solid #e5e7eb;">
                    <div class="pm-task-timer-panel">
                        <div>
                            <h4 style="margin:0 0 4px 0;">Task Timer</h4>
                            <p id="pmb-task-timer-status" class="pm-task-timer-status">Start this task to track time
                                automatically.</p>
                        </div>
                        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                            <span id="pmb-task-timer-elapsed" class="pm-task-timer-elapsed">00:00:00</span>
                            <div id="pmb-task-timer-button-wrap"></div>
                        </div>
                    </div>
                    <hr style="margin:24px 0;border:none;border-top:1px solid #e5e7eb;">
                    <div class="pm-task-logs">
                        <div class="logs-header">
                            <h4>Time Logs</h4>
                            <button type="button" class="bntm-btn-small bntm-btn-secondary" onclick="pmbOpenTimeLogForm()">+
                                Log Time</button>
                        </div>
                        <div id="pmb-time-logs-list"></div>
                        <div id="pmb-time-log-form"
                            style="display:none;margin-top:12px;padding:16px;background:#f9fafb;border-radius:8px;">
                            <div class="bntm-form-row">
                                <div class="bntm-form-group">
                                    <label>Hours *</label>
                                    <input type="number" id="pmb-log-hours" step="0.5" min="0.5" class="bntm-input">
                                </div>
                                <div class="bntm-form-group">
                                    <label>Date *</label>
                                    <input type="date" id="pmb-log-date" value="<?php echo date('Y-m-d'); ?>"
                                        class="bntm-input">
                                </div>
                            </div>
                            <div class="bntm-form-group">
                                <label>Notes</label>
                                <textarea id="pmb-log-notes" rows="2" class="bntm-input"></textarea>
                            </div>
                            <div style="display:flex;gap:8px;">
                                <button type="button" class="bntm-btn-primary" onclick="pmbSaveTimeLog()">Save Log</button>
                                <button type="button" class="bntm-btn-secondary"
                                    onclick="pmbCloseTimeLogForm()">Cancel</button>
                            </div>
                        </div>
                    </div>
                    <hr style="margin:24px 0;border:none;border-top:1px solid #e5e7eb;">
                    <div class="pm-task-comments">
                        <h4>Comments</h4>
                        <div id="pmb-comments-list"></div>
                        <div class="pm-add-comment bntm-form-group">
                            <textarea id="pmb-new-comment" rows="3" placeholder="Add a comment…"
                                class="bntm-input"></textarea>
                            <button type="button" class="bntm-btn-primary" onclick="pmbAddComment()"
                                style="margin-top:8px;">Add Comment</button>
                        </div>
                    </div>
                </div>

                <div class="bntm-modal-footer">
                    <button type="button" class="bntm-btn-secondary" onclick="pmbCloseTaskModal()">Cancel</button>
                    <button type="submit" class="bntm-btn-primary"><span id="pmb-submit-text">Update Task</span></button>
                </div>
            </form>
        </div>
    </div>
    ...
    <style>
        ...
        .pm-board-title-group {
            display: flex;
            align-items: center;
            gap: 8px;
        }

        .pm-board-title-group h2 {
            margin: 0;
            font-size: 18px;
            font-weight: 700;
            color: #111827;
        ...
        .pmb-table th.pmb-sortable {
            cursor: pointer;
            user-select: none;
        }

        .pmb-table th.pmb-sortable:hover {
            color: #374151;
        }
        ...
        .pmb-prio-dot.priority-low {
            background: #3b82f6;
        }

        .pmb-prio-dot.priority-medium {
            background: #f59e0b;
        }

        .pmb-prio-dot.priority-high {
            background: #ef4444;
        }
        ...
        /* ═══════════════════════════════════════════════════════════
           SNIPPET CELLS (comment / log hover triggers)
        ═══════════════════════════════════════════════════════════ */
        .pmb-snippet-cell {
            overflow: visible !important;
        }
        ...
        /* ═══════════════════════════════════════════════════════════
           TOOLTIP — fixed, no scroll, auto-height
        ═══════════════════════════════════════════════════════════ */
        .pmb-snip-tooltip {
            display: none;
            /* hidden by default */
            ...
            overflow: hidden;
            pointer-events: none;
            /* hover goes to trigger, not tooltip */
            animation: pmbFadeIn .14s ease;
        }
        ...
        .pmb-snip-tooltip-author {
            font-size: 12px;
            font-weight: 700;
            color: #e2e8f0;
        }

        .pmb-snip-tooltip-meta {
            font-size: 11px;
            color: #64748b;
            white-space: nowrap;
            flex-shrink: 0;
        ...
        /* ═══════════════════════════════════════════════════════════
           QUICK ACTIONS — visible on row hover only
        ═══════════════════════════════════════════════════════════ */
        .pmb-actions-cell {
            white-space: nowrap;
            display: flex;
        ...
        .pmb-action-btn:hover {
            opacity: 1 !important;
        }

        .pmb-action-edit:hover {
            background: #eff6ff;
            color: #2563eb;
        }

        .pmb-action-delete:hover {
            background: #fef2f2;
            color: #dc2626;
        }

        /* timer button inherits from existing pm_render_task_timer_button styles */
        .pm-task-timer-btn-board,
        .pm-task-timer-btn-row {
            opacity: 0;
            transition: opacity .12s;
        }

        .pmb-row:hover .pm-task-timer-btn-board,
        .pmb-row:hover .pm-task-timer-btn-row {
            opacity: 1;
        }

        .pm-task-timer-btn.is-active,
        .pm-task-timer-btn.is-active.pm-task-timer-btn-board,
        .pm-task-timer-btn.is-active.pm-task-timer-btn-row {
            opacity: 1 !important;
        }
        ...
        .pm-task-timer-status {
            margin: 0;
            font-size: 13px;
            color: #6b7280;
        }

        .pm-task-timer-elapsed {
            display: inline-flex;
            align-items: center;
            justify-content: center;
            min-width: 96px;
    ...
    <script>
        (function () {
            ...
            var dueSortDir = '';
            var pmbTaskTimerInterval = null;
            var pmbActiveTaskId = <?php echo (int) $active_task_id; ?>;

            /* ── Timer helpers ───────────────────────────────────────────── */
            function pmbFormatTimerElapsed(startedAt) {
                if (!startedAt) return '00:00:00';
                var seconds = Math.max(0, Math.floor((Date.now() - new Date(startedAt).getTime()) / 1000));
            ...
            function pmbStopTaskTimerClock() {
                if (pmbTaskTimerInterval) { clearInterval(pmbTaskTimerInterval); pmbTaskTimerInterval = null; }
            }
            function pmbStartTaskTimerClock(startedAt) {
                pmbStopTaskTimerClock();
                var elapsed = document.getElementById('pmb-task-timer-elapsed');
                if (!elapsed || !startedAt) return;
                elapsed.textContent = pmbFormatTimerElapsed(startedAt);
                pmbTaskTimerInterval = setInterval(function () {
                    elapsed.textContent = pmbFormatTimerElapsed(startedAt);
                }, 1000);
            }
            function pmbRenderTaskTimerButton(taskId, isActive) {
                return '<button type="button" class="pm-task-timer-btn pm-task-timer-btn-modal' + (isActive ? ' is-active' : '') + '" data-task-id="' + taskId + '" data-active="' + (isActive ? '1' : '0') + '"><span class="pm-task-timer-btn-dot"></span><span class="pm-task-timer-btn-label">' + (isActive ? 'Active' : 'Inactive') + '</span></button>';
            }
            function pmbSyncTaskTimerButtons(activeTaskId) {
                document.querySelectorAll('.pm-task-timer-btn').forEach(function (btn) {
                    var isActive = String(btn.dataset.taskId) === String(activeTaskId) && String(activeTaskId) !== '0';
                    btn.dataset.active = isActive ? '1' : '0';
                    btn.classList.toggle('is-active', isActive);
                    var label = btn.querySelector('.pm-task-timer-btn-label');
                    if (label) label.textContent = isActive ? 'Active' : 'Inactive';
                });
                document.querySelectorAll('.pmb-row').forEach(function (row) {
                    row.dataset.isActive = String(row.dataset.taskId) === String(activeTaskId) ? '1' : '0';
                });
            }
            function pmbUpdateTaskTimerUi(taskId, timer) {
                var wrap = document.getElementById('pmb-task-timer-button-wrap');
                var status = document.getElementById('pmb-task-timer-status');
                var elapsed = document.getElementById('pmb-task-timer-elapsed');
                var isActive = !!(timer && timer.is_active);
                pmbActiveTaskId = isActive ? taskId : (timer && timer.active_task_id ? parseInt(timer.active_task_id, 10) : 0);
                pmbSyncTaskTimerButtons(pmbActiveTaskId);
                if (wrap) wrap.innerHTML = pmbRenderTaskTimerButton(taskId, isActive);
                if (status) {
                    if (isActive) {
                        status.textContent = 'Timer is running. Stop it to automatically save the elapsed time as a time log.';
                    } else if (pmbActiveTaskId && pmbActiveTaskId !== taskId) {
                        status.textContent = 'Another task is currently active. Starting this one will stop the previous timer automatically.';
                    } else {
                        status.textContent = 'Start this task to track time automatically.';
                    }
                }
                if (elapsed) {
                    if (isActive && timer.started_at) { pmbStartTaskTimerClock(timer.started_at); }
                    else { pmbStopTaskTimerClock(); elapsed.textContent = '00:00:00'; }
                }
            }
            function pmbToggleTaskTimer(taskId, forceAction) {
                var trigger = document.querySelector('.pm-task-timer-btn[data-task-id="' + taskId + '"]');
                var isActive = forceAction ? forceAction === 'stop' : (trigger && trigger.dataset.active === '1');
                var fd = new FormData();
                fd.append('action', isActive ? 'pm_stop_task_timer' : 'pm_start_task_timer');
                fd.append('task_id', taskId);
                if (!isActive) fd.append('client_started_at', new Date().toISOString());
                fd.append('nonce', pmBoardDetailNonce);
                fetch(ajaxurl, { method: 'POST', body: fd }).then(function (r) { return r.json(); }).then(function (json) {
                    if (!json.success) { alert((json.data && json.data.message) || 'Failed to update task timer'); return; }
                    if (!isActive) {
                        pmbActiveTaskId = parseInt(taskId, 10);
                        pmbSyncTaskTimerButtons(taskId);
                        var mid = document.getElementById('pmb-task-id').value;
                        if (mid === String(taskId)) pmbUpdateTaskTimerUi(taskId, { is_active: true, active_task_id: taskId, started_at: json.data.started_at });
                        else if (mid) pmbUpdateTaskTimerUi(parseInt(mid, 10), { is_active: false, active_task_id: taskId, started_at: '' });
                    } else {
                        pmbActiveTaskId = 0;
                        pmbSyncTaskTimerButtons(0);
                        var mids = document.getElementById('pmb-task-id').value;
                        if (mids === String(taskId)) { pmbUpdateTaskTimerUi(taskId, { is_active: false, active_task_id: 0, started_at: '' }); pmbEditTask(taskId); }
                        else if (mids) pmbUpdateTaskTimerUi(parseInt(mids, 10), { is_active: false, active_task_id: 0, started_at: '' });
                    }
                });
            }

            /* ── Filters ─────────────────────────────────────────────────── */
            window.pmbOnProjectChange = function () {
                var projId = document.getElementById('pmb-project-filter').value;
                var statSel = document.getElementById('pmb-status-filter');
                if (!projId) {
                    statSel.style.display = 'none'; statSel.value = '';
                } else {
                    var statuses = pmBoardProjectStatuses[projId] || [];
                    statSel.innerHTML = '<option value="">All Statuses</option>';
                    statuses.forEach(function (sn) {
                        var o = document.createElement('option'); o.value = sn; o.textContent = sn; statSel.appendChild(o);
                    });
                    statSel.style.display = '';
                }
                pmbApplyFilters();
            };
            window.pmbSetAssignment = function (f) {
                assignmentFilter = f;
                document.querySelectorAll('.pm-board-filter-btn').forEach(function (b) { b.classList.toggle('active', b.dataset.filter === f); });
                pmbApplyFilters();
            };
            window.pmbToggleSort = function () {
                var sel = document.getElementById('pmb-deadline-sort');
                dueSortDir = dueSortDir === 'asc' ? 'desc' : 'asc';
                sel.value = dueSortDir;
                document.getElementById('pmb-due-icon').textContent = dueSortDir === 'asc' ? '↑' : '↓';
                pmbApplyFilters();
            };
            window.pmbApplyFilters = function () {
                var projVal = document.getElementById('pmb-project-filter').value;
                var statVal = document.getElementById('pmb-status-filter').value;
                var sortVal = document.getElementById('pmb-deadline-sort').value;
                var search = document.getElementById('pmb-search').value.toLowerCase().trim();
                dueSortDir = sortVal;

                var tbody = document.getElementById('pmb-tbody');
                var rows = Array.from(tbody.querySelectorAll('.pmb-row'));
                rows.forEach(function (row) {
                    var pid = row.dataset.projectId;
                    var ato = row.dataset.assignedTo;
                    var stat = row.dataset.taskStatus;
                    var title = (row.querySelector('.pmb-task-title') || {}).textContent || '';
                    var proj = (row.querySelector('.pmb-project-link') || {}).textContent || '';
                    var show =
                        (!projVal || pid === projVal) &&
                        (!statVal || stat === statVal) &&
                        (assignmentFilter !== 'my' || String(ato) === String(pmBoardCurrentUserId)) &&
                        (!search || title.toLowerCase().includes(search) || proj.toLowerCase().includes(search));
                    row.classList.toggle('pmb-hidden', !show);
                });
                if (sortVal === 'asc' || sortVal === 'desc') {
                    var vis = rows.filter(function (r) { return !r.classList.contains('pmb-hidden'); });
                    vis.sort(function (a, b) {
                        var ta = parseInt(a.dataset.dueTs) || 0, tb = parseInt(b.dataset.dueTs) || 0;
                        if (!ta && !tb) return 0; if (!ta) return 1; if (!tb) return -1;
                        return sortVal === 'asc' ? ta - tb : tb - ta;
                    });
                    vis.forEach(function (r) { tbody.appendChild(r); });
                }
                var visCount = rows.filter(function (r) { return !r.classList.contains('pmb-hidden'); }).length;
                document.getElementById('pmb-visible-count-badge').textContent = visCount;
                document.getElementById('pmb-no-results').style.display = visCount === 0 ? 'block' : 'none';
            };

            /* ══════════════════════════════════════════════════════════════
               TOOLTIP — FIXED (hover shows, mouseout hides correctly)
            ══════════════════════════════════════════════════════════════ */
            var tooltip = document.getElementById('pmb-snip-tooltip');
            var tooltipOpen = null;
            var hideTimer = null;

            function pmbShowTooltip(trigger) {
                clearTimeout(hideTimer);
                var raw = trigger.dataset.tooltip;
                if (!raw) return;
                var data;
                try { data = JSON.parse(raw); } catch (e) { return; }

                var isLog = trigger.classList.contains('pmb-snip-log');

                tooltip.querySelector('.pmb-snip-tooltip-author').textContent = data.author || '';

                if (isLog) {
                    tooltip.querySelector('.pmb-snip-tooltip-meta').textContent =
                        (data.hours ? data.hours + 'h' : '') +
                        (data.date ? ' · ' + new Date(data.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '');
                    tooltip.querySelector('.pmb-snip-tooltip-body').textContent = data.notes || '(no notes)';
                } else {
                    tooltip.querySelector('.pmb-snip-tooltip-meta').textContent =
                        data.created ? pmbTimeAgo(data.created) : '';
                    tooltip.querySelector('.pmb-snip-tooltip-body').textContent = data.text || '';
                }
                ...
                var top = tr.bottom + 8;
                var left = tr.left;
                if (top + th > vh - 10) top = tr.top - th - 8;
            ...
            function pmbHideTooltip() {
                tooltip.style.display = 'none';
                tooltipOpen = null;
            }

            /*
             * FIX: Use a small delay so moving the mouse from trigger → tooltip
             * doesn't flicker. The tooltip has pointer-events:none so this is
             * actually not needed for mousing over it, but the delay gives a
             * smoother feel when quickly moving between cells.
             * The key fix is: mouseleave on the trigger fires correctly because
             * we do NOT call pmbHideTooltip() blindly in a global mouseover.
             */
            document.addEventListener('mouseover', function (e) {
                var t = e.target.closest('.pmb-snip-wrap');
                if (t) { pmbShowTooltip(t); }
            });

            document.addEventListener('mouseout', function (e) {
                var from = e.target.closest('.pmb-snip-wrap');
                if (!from) return;                          /* we only care about leaving a trigger */
                var to = e.relatedTarget ? e.relatedTarget.closest('.pmb-snip-wrap') : null;
                if (to !== from) {
                    /* leaving the trigger (not entering another child of same trigger) */
                    hideTimer = setTimeout(pmbHideTooltip, 80);
                }
            });

            document.addEventListener('keydown', function (e) {
                if (e.key === 'Escape') pmbHideTooltip();
            });

            /* ── Delegated click handler ─────────────────────────────────── */
            document.addEventListener('click', function (e) {
                if (e.target.closest('.pm-task-status-btn')) {
                    var btn = e.target.closest('.pm-task-status-btn'), menu = btn.nextElementSibling;
                    document.querySelectorAll('.task-status-menu').forEach(function (m) { if (m !== menu) m.classList.remove('show'); });
                    menu.classList.toggle('show'); return;
                }
                if (e.target.closest('.pm-task-status-option')) {
                    var o = e.target.closest('.pm-task-status-option');
                    pmbSetStatus(o.dataset.taskId, o.dataset.status, o.dataset.color, o.textContent.trim()); return;
                }
                if (e.target.closest('.pm-edit-task-btn')) { pmbEditTask(e.target.closest('.pm-edit-task-btn').dataset.taskId); return; }
                if (e.target.closest('.pm-task-timer-btn')) { pmbToggleTaskTimer(e.target.closest('.pm-task-timer-btn').dataset.taskId); return; }
                if (e.target.closest('.pm-delete-task-btn')) { var b = e.target.closest('.pm-delete-task-btn'); pmbDeleteTask(b.dataset.taskId, b.dataset.taskTitle); return; }
                if (e.target.closest('.pmb-delete-log')) { var b = e.target.closest('.pmb-delete-log'); pmbDeleteTimeLog(b.dataset.logId, b.dataset.taskId); return; }
                if (e.target.closest('.pmb-delete-comment')) { var b = e.target.closest('.pmb-delete-comment'); pmbDeleteComment(b.dataset.commentId, b.dataset.taskId); return; }
                if (!e.target.closest('.task-status-dropdown')) {
                    document.querySelectorAll('.task-status-menu').forEach(function (m) { m.classList.remove('show'); });
                }
            }, true);

            function pmbSetStatus(taskId, status, color, label) {
                var row = document.querySelector('.pmb-row[data-task-id="' + taskId + '"]');
                if (row) {
                    row.querySelector('.task-status-menu').classList.remove('show');
                    row.setAttribute('data-task-status', status);
                    var btn = row.querySelector('.task-status-btn');
                    if (btn) { btn.style.background = color; btn.querySelector('span').textContent = label; }
                }
                var fd = new FormData();
                fd.append('action', 'pm_update_task_status'); fd.append('task_id', taskId);
                fd.append('status', status); fd.append('nonce', pmBoardDetailNonce);
                fetch(ajaxurl, { method: 'POST', body: fd });
            }

            function pmbEditTask(taskId) {
                var row = document.querySelector('.pmb-row[data-task-id="' + taskId + '"]');
                var milestones = row ? JSON.parse(row.dataset.milestones || '[]') : [];
                var team = row ? JSON.parse(row.dataset.team || '[]') : [];
                var statuses = row ? JSON.parse(row.dataset.statuses || '[]') : [];
                pmbPopulateDropdowns(statuses, team, milestones);
                var fd = new FormData();
                fd.append('action', 'pm_get_task_details'); fd.append('task_id', taskId); fd.append('nonce', pmBoardDetailNonce);
                fetch(ajaxurl, { method: 'POST', body: fd }).then(function (r) { return r.json(); }).then(function (json) {
                    if (!json.success) return;
                    var t = json.data.task;
                    pmBoardActiveProjectId = t.project_id;
                    document.getElementById('pmb-task-id').value = taskId;
                    document.getElementById('pmb-task-project-id').value = t.project_id;
                    document.getElementById('pmb-task-title').value = t.title;
                    document.getElementById('pmb-task-desc').value = t.description || '';
                    document.getElementById('pmb-task-status').value = t.status;
                    document.getElementById('pmb-task-priority').value = t.priority;
                    document.getElementById('pmb-task-assigned').value = t.assigned_to || '';
                    document.getElementById('pmb-task-milestone').value = t.milestone_id || '';
                    document.getElementById('pmb-task-due').value = t.due_date || '';
                    document.getElementById('pmb-task-estimated').value = t.estimated_hours || '';
                    document.getElementById('pmb-task-tags').value = t.tags || '';
                    document.getElementById('pmb-modal-title').textContent = 'Edit Task';
                    document.getElementById('pmb-submit-text').textContent = 'Update Task';
                    document.getElementById('pmb-task-logs-section').style.display = 'block';
                    pmbLoadTimeLogs(taskId, json.data.time_logs);
                    pmbLoadComments(taskId, json.data.comments);
                    pmbUpdateTaskTimerUi(taskId, json.data.timer || {});
                    document.getElementById('pm-board-task-modal').style.display = 'flex';
                });
            }

            function pmbPopulateDropdowns(statuses, team, milestones) {
                var ss = document.getElementById('pmb-task-status'); ss.innerHTML = '';
                (statuses.length ? statuses : [{ status_name: 'To Do' }, { status_name: 'In Progress' }, { status_name: 'Review' }, { status_name: 'completed' }])
                    .forEach(function (s) { var o = document.createElement('option'); o.value = s.status_name; o.textContent = s.status_name; ss.appendChild(o); });
                var as = document.getElementById('pmb-task-assigned'); as.innerHTML = '<option value="">Unassigned</option>';
                team.forEach(function (m) { var o = document.createElement('option'); o.value = m.user_id; o.textContent = m.display_name; as.appendChild(o); });
                var ms = document.getElementById('pmb-task-milestone'); ms.innerHTML = '<option value="">No Milestone</option>';
                milestones.forEach(function (m) { var o = document.createElement('option'); o.value = m.id; o.textContent = m.name; ms.appendChild(o); });
            }

            window.pmbCloseTaskModal = function () { document.getElementById('pm-board-task-modal').style.display = 'none'; pmbStopTaskTimerClock(); };

            document.getElementById('pm-board-task-form').addEventListener('submit', function (e) {
                e.preventDefault();
                var fd = new FormData(this), taskId = document.getElementById('pmb-task-id').value;
                fd.append('action', taskId ? 'pm_update_task' : 'pm_create_task'); fd.append('nonce', pmBoardDetailNonce);
                var btn = this.querySelector('button[type="submit"]'); btn.disabled = true;
                document.getElementById('pmb-submit-text').textContent = taskId ? 'Updating…' : 'Creating…';
                fetch(ajaxurl, { method: 'POST', body: fd }).then(function (r) { return r.json(); }).then(function (json) {
                    if (json.success) { location.reload(); }
                    else { alert(json.data.message || 'Failed'); btn.disabled = false; document.getElementById('pmb-submit-text').textContent = taskId ? 'Update Task' : 'Create Task'; }
                });
            });

            function pmbDeleteTask(taskId, title) {
                if (!confirm('Delete task "' + title + '"?')) return;
                var fd = new FormData(); fd.append('action', 'pm_delete_task'); fd.append('task_id', taskId); fd.append('nonce', pmBoardDetailNonce);
                fetch(ajaxurl, { method: 'POST', body: fd }).then(function (r) { return r.json(); }).then(function (json) {
                    if (json.success) { var r = document.querySelector('.pmb-row[data-task-id="' + taskId + '"]'); if (r) r.remove(); pmbApplyFilters(); }
                    else alert(json.data.message || 'Failed');
                });
            }

            /* ── Time logs ───────────────────────────────────────────────── */
            window.pmbOpenTimeLogForm = function () { document.getElementById('pmb-time-log-form').style.display = 'block'; };
            window.pmbCloseTimeLogForm = function () { document.getElementById('pmb-time-log-form').style.display = 'none'; document.getElementById('pmb-log-hours').value = ''; document.getElementById('pmb-log-notes').value = ''; };
            window.pmbSaveTimeLog = function () {
                var taskId = document.getElementById('pmb-task-id').value, hours = document.getElementById('pmb-log-hours').value, logDate = document.getElementById('pmb-log-date').value, notes = document.getElementById('pmb-log-notes').value;
                if (!hours || !logDate) { alert('Please fill in Hours and Date'); return; }
                var fd = new FormData(); fd.append('action', 'pm_add_time_log'); fd.append('task_id', taskId); fd.append('hours', hours); fd.append('log_date', logDate); fd.append('notes', notes); fd.append('nonce', pmBoardDetailNonce);
                fetch(ajaxurl, { method: 'POST', body: fd }).then(function (r) { return r.json(); }).then(function (json) { if (json.success) { pmbCloseTimeLogForm(); pmbEditTask(taskId); } else alert(json.data.message || 'Failed'); });
            };
            function pmbDeleteTimeLog(logId, taskId) {
                if (!confirm('Delete this time log?')) return;
                var fd = new FormData(); fd.append('action', 'pm_delete_time_log'); fd.append('log_id', logId); fd.append('nonce', pmBoardDetailNonce);
                fetch(ajaxurl, { method: 'POST', body: fd }).then(function (r) { return r.json(); }).then(function (json) { if (json.success) pmbEditTask(taskId); else alert('Failed'); });
            }
            function pmbLoadTimeLogs(taskId, logs) {
                var c = document.getElementById('pmb-time-logs-list');
                if (!logs || !logs.length) { c.innerHTML = '<p style="text-align:center;color:#9ca3af;padding:14px;font-size:13px;">No time logs yet</p>'; return; }
                c.innerHTML = logs.map(function (l) {
                    var d = new Date(l.log_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
                    return '<div class="pm-time-log-item"><div class="time-log-info"><p><strong>' + (l.user_name || 'Unknown') + '</strong> — <span class="time-log-hours">' + l.hours + 'h</span></p><small>' + d + (l.notes ? ' · ' + l.notes : '') + '</small></div>'
                        + '<button class="bntm-btn-small bntm-btn-danger pmb-delete-log" data-log-id="' + l.id + '" data-task-id="' + taskId + '"><svg width="11" height="11" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div>';
                }).join('');
            }

            /* ── Comments ────────────────────────────────────────────────── */
            window.pmbAddComment = function () {
                var taskId = document.getElementById('pmb-task-id').value, comment = document.getElementById('pmb-new-comment').value.trim();
                if (!comment) { alert('Please enter a comment'); return; }
                var fd = new FormData(); fd.append('action', 'pm_add_comment'); fd.append('task_id', taskId); fd.append('comment', comment); fd.append('nonce', pmBoardDetailNonce);
                fetch(ajaxurl, { method: 'POST', body: fd }).then(function (r) { return r.json(); }).then(function (json) { if (json.success) { document.getElementById('pmb-new-comment').value = ''; pmbEditTask(taskId); } else alert(json.data.message || 'Failed'); });
            };
            function pmbDeleteComment(commentId, taskId) {
                if (!confirm('Delete this comment?')) return;
                var fd = new FormData(); fd.append('action', 'pm_delete_comment'); fd.append('comment_id', commentId); fd.append('nonce', pmBoardDetailNonce);
                fetch(ajaxurl, { method: 'POST', body: fd }).then(function (r) { return r.json(); }).then(function (json) { if (json.success) pmbEditTask(taskId); else alert('Failed'); });
            }
            function pmbLoadComments(taskId, comments) {
                var c = document.getElementById('pmb-comments-list');
                if (!comments || !comments.length) { c.innerHTML = '<p style="text-align:center;color:#9ca3af;padding:14px;font-size:13px;">No comments yet</p>'; return; }
                c.innerHTML = comments.map(function (cm) {
                    var ago = pmbTimeAgo(cm.created_at), div = document.createElement('div'); div.textContent = cm.comment;
                    return '<div class="pm-comment-item"><div class="comment-header"><span class="comment-author">' + (cm.user_name || 'Unknown') + '</span>'
                        + '<div style="display:flex;align-items:center;gap:8px;"><span class="comment-time">' + ago + '</span>'
                        + '<button class="bntm-btn-small bntm-btn-danger pmb-delete-comment" data-comment-id="' + cm.id + '" data-task-id="' + taskId + '" style="padding:3px 5px;">'
                        + '<svg width="10" height="10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button></div></div>'
                        + '<p class="comment-text" style="white-space:pre-line;word-break:break-word;">' + div.innerHTML.replace(/\n/g, '<br>') + '</p></div>';
                }).join('');
            }

            function pmbTimeAgo(ds) {
                var s = Math.floor((Date.now() - new Date(ds)) / 1000), iv = { year: 31536000, month: 2592000, week: 604800, day: 86400, hour: 3600, minute: 60 };
                for (var u in iv) { var n = Math.floor(s / iv[u]); if (n >= 1) return n + ' ' + u + (n > 1 ? 's' : '') + ' ago'; } return 'just now';
            }

            /* ── Drag-and-drop reorder ───────────────────────────────────── */
            (function () {
                var dragged = null, tbody = document.getElementById('pmb-tbody');
                document.addEventListener('dragstart', function (e) { var r = e.target.closest('.pmb-row'); if (r) { dragged = r; setTimeout(function () { r.classList.add('dragging'); }, 0); } }, true);
                document.addEventListener('dragend', function () { if (dragged) { dragged.classList.remove('dragging'); dragged = null; } }, true);
                document.addEventListener('dragover', function (e) {
                    if (!dragged) return;
                    var r = e.target.closest('.pmb-row'); if (!r || r === dragged) return; e.preventDefault();
                    var rect = r.getBoundingClientRect();
                    e.clientY < rect.top + rect.height / 2 ? tbody.insertBefore(dragged, r) : tbody.insertBefore(dragged, r.nextSibling);
                }, true);
                document.addEventListener('drop', function (e) {
                    if (!dragged) return; e.preventDefault();
                    var byProject = {};
                    Array.from(tbody.querySelectorAll('.pmb-row')).forEach(function (r) {
                        var pid = r.dataset.projectId; if (!byProject[pid]) byProject[pid] = [];
                        byProject[pid].push(r.dataset.taskId);
                    });
                    Object.keys(byProject).forEach(function (pid) {
                        var fd = new FormData(); fd.append('action', 'pm_reorder_tasks'); fd.append('milestone_id', 0);
                        fd.append('task_order', JSON.stringify(byProject[pid])); fd.append('nonce', pmBoardReorderNonce);
                        fetch(ajaxurl, { method: 'POST', body: fd });
                    });
                }, true);
            })();
...
function pm_resource_load_tab($business_id)
{
    ...
    $active_task_sessions = $wpdb->get_results($wpdb->prepare(
        "SELECT s.task_id, s.user_id, s.started_at,
                t.title as task_title, t.status as task_status, t.priority, t.project_id,
                p.name as project_name, p.color as project_color,
                u.display_name, u.user_email
         FROM {$sessions_table} s
         INNER JOIN {$tasks_table} t ON s.task_id = t.id
         INNER JOIN {$projects_table} p ON t.project_id = p.id
         INNER JOIN {$wpdb->users} u ON s.user_id = u.ID
         WHERE 1=1 {$project_filter} {$vis['where']}
    ...
    foreach ($team_workload as $m) {
        if ($m->estimated_hours > $overloaded_threshold)
            $overloaded++;
        elseif ($m->estimated_hours >= $optimal_threshold)
            $optimal++;
        else
            $underutilized++;
    }
    ...
    <div class="pm-resource-load-container">
            ...
            <div class="pm-capacity-stats">
                ...
                <div class="pm-capacity-card optimal">
                    ...
                    <div>
                        ...
                        <p class="capacity-label">Team members
                            (<?php echo $optimal_threshold; ?>–<?php echo $overloaded_threshold; ?>h)</p>
                    </div>
                </div>
                ...
                <div class="pm-user-legend">
                    <div style="width:100%;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid #e5e7eb;">
                        ...
                        <span style="margin-left:16px;font-size:11px;">
                            <span
                                style="display:inline-block;width:20px;height:12px;background:#3b82f6;border:2px solid rgba(0,0,0,0.1);border-radius:2px;vertical-align:middle;"></span>
                            Estimated
                        </span>
                        <span style="margin-left:12px;font-size:11px;">
                            <span
                                style="display:inline-block;width:20px;height:12px;background:#3b82f6;opacity:0.6;border:2px dashed rgba(255,255,255,0.5);border-radius:2px;vertical-align:middle;"></span>
                            Actual
                        </span>
                    </div>
                                        ...
                                        foreach ($day_data['users'] as $ut): ?>
                                            <div class="user-task-group">
                                                <div class="user-task-bar estimated"
                                                    ...
                                                    data-tooltip="<?php echo esc_attr($ut['user_name'] . ' - Estimated: ' . number_format($ut['estimated_hours'], 1) . 'h'); ?>">
                                                </div>
                                                <div class="user-task-bar actual"
                                                    style="background:<?php echo $ut['color']; ?>;opacity:0.6;height:<?php echo max($ut['actual_height'], 5); ?>px;"
                                                    data-tooltip="<?php echo esc_attr($ut['user_name'] . ' - Actual: ' . number_format($ut['actual_hours'], 1) . 'h'); ?>">
                                                </div>
                                            </div>
                                        <?php endforeach;
                                    else: ?>
        ...
        <div class="pm-resource-section">
            ...
            <div class="pm-active-task-table-wrap">
                <table class="pm-active-task-table">
                    <thead>
                        <tr>
                            <th>Task</th>
                            <th>Project</th>
                    ...
                    <tbody>
                        <?php if (!empty($active_task_sessions)): ?>
                            <?php foreach ($active_task_sessions as $session):
                                $started_at_iso = pm_utc_mysql_to_iso8601($session->started_at);
                                $elapsed_seconds = max(0, current_time('timestamp', true) - strtotime($started_at_iso));
                                $elapsed_label = gmdate('H:i:s', $elapsed_seconds);
                                ?>
                                <tr>
                                    <td>
                                        <div class="pm-active-task-title-cell">
                                            <span
                                                class="pm-active-priority priority-<?php echo esc_attr($session->priority ?: 'medium'); ?>"></span>
                                            <div>
                                                <strong><?php echo esc_html($session->task_title); ?></strong>
                                                <div style="font-size:12px;color:#6b7280;">Task
                                                    #<?php echo (int) $session->task_id; ?></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <span class="pm-active-project-chip"
                                            style="--pm-project-color: <?php echo esc_attr($session->project_color ?: '#3b82f6'); ?>;">
                                            <?php echo esc_html($session->project_name); ?>
                                        </span>
                                    </td>
                                    <td>
                                        <div style="display:flex;align-items:center;gap:8px;">
                                            <div class="resource-avatar"
                                                style="background:<?php echo esc_attr($user_color_map[$session->user_id]['color'] ?? '#6b7280'); ?>;">
                                                <?php echo esc_html(strtoupper(substr($session->display_name, 0, 2))); ?>
                                            </div>
                                            <div>
                                                <strong><?php echo esc_html($session->display_name); ?></strong>
                                                <div style="font-size:12px;color:#6b7280;">
                                                    <?php echo esc_html($session->user_email); ?></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td><span class="resource-badge"><?php echo esc_html($session->task_status); ?></span></td>
                                    <td>
                                        <span class="pm-active-started-local"
                                            data-started-at="<?php echo esc_attr($started_at_iso); ?>">
                                            <?php echo esc_html($started_at_iso); ?>
                                        </span>
                                    </td>
                                    <td><span class="pm-active-elapsed"><?php echo esc_html($elapsed_label); ?></span></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="6" style="text-align:center;padding:40px;color:#9ca3af;">No active tasks right now
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                                ...
                                <tr>
                                    <td>
                                        <div style="display:flex;align-items:center;gap:8px;">
                                            ...
                                            <div>
                                                ...
                                                <div style="font-size:12px;color:#6b7280;">
                                                    <?php echo esc_html($member->user_email); ?></div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <?php if ($member->high_priority_tasks > 0): ?>
                                            <span
                                                class="resource-badge high-priority"><?php echo $member->high_priority_tasks; ?></span>
                                        <?php else: ?>
                                            <span style="color:#9ca3af;">—</span>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td><span
                                            style="color:#059669;"><?php echo number_format($member->logged_hours, 1); ?>h</span>
                                    </td>
                                    <td>
                                        <div class="capacity-indicator">
                                            ...
                                            <span
                                                class="capacity-text <?php echo $cap_class; ?>"><?php echo round($cap_pct); ?>%</span>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach;
                    ...
                    <?php foreach ($project_distribution as $project): ?>
                        <div class="pm-project-resource-card">
                            <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
                                <div
                                    style="width:4px;height:32px;background:<?php echo esc_attr($project->color); ?>;border-radius:2px;">
                                </div>
        ...
        <div class="pm-additional-analytics">
            ...
            <div class="pm-insights-grid">
                ...
                <div class="pm-insight-card">
                    ...
                    <p class="insight-value">
                        <?php echo number_format(array_sum(array_column($team_workload, 'logged_hours')), 1); ?>h</p>
                </div>
                <div class="pm-insight-card">
    ...
    <style>
        ...
        .pm-active-task-table-wrap {
            overflow-x: auto;
            border: 1px solid #e5e7eb;
            border-radius: 8px;
        }

        .pm-active-task-table {
            width: 100%;
            border-collapse: collapse;
            font-size: 13px;
            min-width: 900px;
        ...
        .pm-active-task-table thead tr {
            background: #f9fafb;
            border-bottom: 2px solid #e5e7eb;
        }

        .pm-active-task-table th {
            padding: 10px 12px;
            text-align: left;
            font-size: 11px;
            font-weight: 600;
        ...
        .pm-active-task-table td {
            padding: 12px;
            border-bottom: 1px solid #f3f4f6;
            vertical-align: middle;
        }

        .pm-active-task-table tbody tr:last-child td {
            border-bottom: none;
        }

        .pm-active-task-table tbody tr:hover {
            background: #fafafa;
        }

        .pm-active-task-title-cell {
            display: flex;
            align-items: center;
            gap: 10px;
        }
        ...
        .pm-active-priority.priority-low {
            background: #3b82f6;
        }

        .pm-active-priority.priority-medium {
            background: #f59e0b;
        }

        .pm-active-priority.priority-high {
            background: #ef4444;
        }

        .pm-active-project-chip {
            display: inline-flex;
            align-items: center;
            gap: 8px;
            padding: 4px 10px;
            border-radius: 999px;
            background: color-mix(in srgb, var(--pm-project-color) 12%, white);
            color: #111827;
            font-weight: 600;
        }

        .pm-active-project-chip::before {
            content: '';
            width: 8px;
            height: 8px;
            border-radius: 999px;
            background: var(--pm-project-color);
        }
    ...
    <script>
        ...
        document.querySelectorAll('.pm-active-started-local').forEach(function (el) {
            ...
            <div class="pm-personal-stats">
                ...
                <div class="pm-personal-stat-card">
                    ...
                    <div>
                        ...
                        <p class="personal-stat-label">
                            <?php echo $personal_tasks > 0 ? round(($personal_completed / $personal_tasks) * 100) : 0; ?>%
                            rate</p>
                    </div>
                </div>
                <div class="pm-personal-stat-card">
                        ...
                        <?php foreach ($personal_projects as $proj):
                            ...
                            <div class="pm-personal-project-card">
                                <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
                                    <div
                                        style="width:3px;height:24px;background:<?php echo esc_attr($proj->color); ?>;border-radius:2px;">
                                    </div>
                                ...
                                <div class="pm-mini-progress" style="margin-top:8px;">
                                    <div class="pm-mini-progress-fill"
                                        style="width:<?php echo $completion; ?>%;background:<?php echo esc_attr($proj->color); ?>">
                                    </div>
                                </div>
                            </div>
                        <?php endforeach; ?>
            ...
            <div class="pm-reports-stats">
                ...
                <div class="pm-report-stat-card">
                    ...
                    <div>
                        ...
                        <p class="stat-value">
                            <?php echo $total_tasks > 0 ? round(($completed_tasks / $total_tasks) * 100) : 0; ?>%</p>
                        ...
                        <?php if (!empty($tasks_by_status)):
                            ...
                            foreach ($tasks_by_status as $stat):
                                ...
                                <div class="status-chart-item">
                                    <span
                                        class="status-chart-label"><?php echo esc_html(ucfirst(str_replace('_', ' ', $stat->status))); ?></span>
                                    <div class="status-chart-bar">
                                        <div class="status-chart-fill"
                                            style="width:<?php echo $pct; ?>%;background:<?php echo pm_get_status_color($stat->status); ?>">
                                        </div>
                                    </div>
                            ...
                            foreach ($tasks_by_priority as $stat): ?>
                                <div class="priority-chart-item">
                                    <div class="priority-chart-header">
                                        <span class="priority-chart-badge"
                                            style="background:<?php echo pm_get_priority_color($stat->priority); ?>">
                                ...
                                foreach ($project_stats as $proj):
                                    ...
                                    <tr>
                                        <td>
                                            <div style="display:flex;align-items:center;gap:8px;">
                                                <div
                                                    style="width:4px;height:30px;background:<?php echo esc_attr($proj->color); ?>;border-radius:2px;">
                                                </div>
                                                <strong><?php echo esc_html($proj->name); ?></strong>
                                            </div>
                                        </td>
                                        <td><span class="pm-status-badge"
                                                style="background:<?php echo pm_get_status_color($proj->status); ?>"><?php echo esc_html(ucfirst($proj->status)); ?></span>
                                        </td>
                                        ...
                                        <td>
                                            <div style="display:flex;align-items:center;gap:8px;">
                                                <div class="pm-mini-progress" style="flex:1;max-width:150px;">
                                                    <div class="pm-mini-progress-fill"
                                                        style="width:<?php echo $comp; ?>%;background:<?php echo esc_attr($proj->color); ?>">
                                                    </div>
                                                </div>
                            ...
                            <div class="pm-team-member-card">
                                <div class="team-member-header">
                                    <div class="team-member-avatar">
                                        <?php echo esc_html(strtoupper(substr($member->display_name, 0, 2))); ?></div>
                                    <div>
                                ...
                                <div class="team-member-stats">
                                    <div class="team-stat"><span class="team-stat-label">Completed</span><span
                                            class="team-stat-value"><?php echo $member->completed_tasks; ?></span></div>
                                    <div class="team-stat"><span class="team-stat-label">Hours</span><span
                                            class="team-stat-value"><?php echo number_format($member->total_hours, 1); ?></span></div>
                                    <div class="team-stat"><span class="team-stat-label">Rate</span><span
                                            class="team-stat-value"><?php echo $mc; ?>%</span></div>
                                </div>
                                <div class="team-member-progress">
                                    <div class="progress-bar">
                                        <div class="progress-fill" style="width:<?php echo $mc; ?>%;background:#3b82f6;"></div>
                                    </div>
                                </div>
                            </div>
                        <?php endforeach; ?>
...
function pm_logs_tab($business_id)
{
    ...
    if (!$can_access) {
        ...
        <div style="background: white; border-radius: 12px; padding: 60px 24px; text-align: center;">
            ...
            <h3 style="margin: 0 0 8px 0; font-size: 20px; color: #374151;">Access Restricted</h3>
            <p style="margin: 0; color: #6b7280; font-size: 15px;">You don't have permission to view activity logs. This feature
                is only available to administrators, owners, and managers.</p>
        </div>
        <?php
        return ob_get_clean();
    }
            ...
            <div class="pm-filter-row bntm-form-group">
                <div class="pm-filter-group ">
                    ...
                    <div style="display: flex; gap: 8px; align-items: center;">
                        <input type="date" id="pm-log-date-from" value="<?php echo esc_attr($date_from); ?>"
                            class="bntm-input">
                        <span>to</span>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if ($log->project_name): ?>
                                        <div style="display: flex; align-items: center; gap: 8px;">
                                            <div
                                                style="width: 3px; height: 24px; background: <?php echo esc_attr($log->project_color); ?>; border-radius: 2px;">
                                            </div>
                                ...
                                <td>
                                    <div class="log-action-badge"
                                        style="background: <?php echo $action_color; ?>15; color: <?php echo $action_color; ?>;">
        ...
        <?php if ($total_pages > 1): ?>
            <div class="pm-logs-pagination">
                <div class="pagination-info">
                    Showing <?php echo (($page - 1) * $per_page) + 1; ?> - <?php echo min($page * $per_page, $total_logs); ?> of
    ...
    <div class="pm-project-detail">
        ...
        <div class="pm-sidebar" id="pm-sidebar">
            ...
            <div class="pm-sidebar-content">
                <div class="pm-project-sidebar-header"
                    style="border-left: 3px solid transparent; display: flex ; gap: 20px; align-items: center;">
                    <div class="project-color-shape"
                        style="width: 30px; height: 30px;background: <?php echo esc_attr($project->color); ?>"></div>
                    <div
                        style="display: flex ; gap: 10px; align-items: center; align-content: center; justify-content: center; flex-direction: column;">
                ...
                <nav class="pm-sidebar-nav">
                    <a href="?project_id=<?php echo $project_id; ?>&subtab=overview" class="pm-sidebar-link <?php echo $subtab === "overview"
                           ? "active"
                           : ""; ?>">
                        <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="?project_id=<?php echo $project_id; ?>&subtab=tasks" class="pm-sidebar-link <?php echo $subtab === "tasks"
                           ? "active"
                           : ""; ?>">
                        <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="?project_id=<?php echo $project_id; ?>&subtab=kanban" class="pm-sidebar-link <?php echo $subtab === "kanban"
                           ? "active"
                           : ""; ?>">
                        <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="?project_id=<?php echo $project_id; ?>&subtab=team" class="pm-sidebar-link <?php echo $subtab === "team"
                           ? "active"
                           : ""; ?>">
                        <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="?project_id=<?php echo $project_id; ?>&subtab=settings" class="pm-sidebar-link <?php echo $subtab === "settings"
                           ? "active"
                           : ""; ?>">
                        <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        if (!empty($all_projects)):
                            foreach ($all_projects as $proj):
                                ...
                                <a href="?project_id=<?php echo $proj->id; ?>&subtab=<?php echo $subtab; ?>"
                                    ...
                                    <div class="project-color-shape" style="background: <?php echo esc_attr($proj->color); ?>">
                                    </div>
...
function pm_project_overview_subtab($project, $business_id)
{
    ...
    <style>
        ...
        < !-- Add these styles to the existing <style>section at the bottom: -->.pm-overview-layout {
            display: grid;
...
function pm_project_tasks_subtab($project, $business_id, $nonce)
{
            ...
            <div style="display: flex; gap: 16px; align-items: center; flex-wrap: wrap;">
                ...
                <div class="pm-tasks-actions">
                    ...
                    <button class="bntm-btn-secondary" onclick="pmExportTasks()">
                        <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1M16 8l-4 4m0 0L8 8m4 4V3" />
                        </svg>
                        Export Tasks
                    </button>
                ...
                <div class="pm-milestone-header" onclick="pmToggleMilestone(this)">
                    ...
                    <div class="milestone-actions" onclick="event.stopPropagation();">
                        ...
                        <div class="milestone-progress-mini">
                            <div class="progress-bar">
                                <div class="progress-fill"
                                    style="height:100%;border-radius:3px;width: <?php echo $milestone_progress; ?>%; background: <?php echo esc_attr($project->color); ?>">
                                </div>
                            </div>
                            <span><?php echo $milestone_progress; ?>%</span>
                        </div>
                        <button class="bntm-btn-small bntm-btn-secondary"
                            onclick="pmEditMilestone(<?php echo htmlspecialchars(json_encode($milestone)); ?>)">
                            <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                        ...
                        <button class="bntm-btn-small bntm-btn-danger"
                            onclick="pmDeleteMilestone(<?php echo $milestone->id; ?>, '<?php echo esc_js($milestone->name); ?>')">
                            <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                            </svg>
                ...
                <div class="pm-tasks-list sortable-milestone-tasks" data-milestone-id="<?php echo $milestone->id; ?>">
                    ...
                    <?php else: ?>
                        <?php foreach ($milestone_tasks as $task):
                            echo pm_render_task_row($task, $team_members, $custom_statuses, $nonce, $active_task_id);
                        endforeach; ?>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <form id="pm-task-form" class="bntm-form">
                ...
                <div id="pm-task-logs-section" style="display: none;">
                    ...
                    <div class="pm-task-timer-panel">
                        <div>
                            <h4 style="margin:0 0 4px 0;">Task Timer</h4>
                            <p id="pm-task-timer-status" class="pm-task-timer-status">Start this task to track time
                                automatically.</p>
                        </div>
                        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                            <span id="pm-task-timer-elapsed" class="pm-task-timer-elapsed">00:00:00</span>
                            <div id="pm-task-timer-button-wrap"></div>
                        </div>
                    </div>
                    ...
                    <div class="pm-task-logs">
                        ...
                        <div id="pm-time-log-form"
                            ...
                            <div style="display: flex; gap: 8px;">
                                ...
                                <button type="button" class="bntm-btn-secondary"
                                    onclick="pmCloseTimeLogForm()">Cancel</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="pm-task-comments">
                        ...
                        <div class="pm-add-comment bntm-form-group">
                            <textarea id="pm-new-comment" rows="3" placeholder="Add a comment..."
                                class="bntm-input"></textarea>
                            <button type="button" class="bntm-btn-primary" onclick="pmAddComment()"
                ...
                <div class="bntm-modal-footer">
                    <div style="display: flex; gap: 12px; align-items: center; flex: 1;">
                        <button type="button" id="pm-export-google-calendar-btn" class="bntm-btn-secondary"
                            onclick="pmExportToGoogleCalendar()" style="display: none; gap: 8px;">
                            <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
            ...
            <div class="bntm-form">
                <div class="bntm-form-group">
                    ...
                    <small style="color: #6b7280; display: block; margin-top: 8px;">
                        Expected CSV headers: title, description, status, priority, milestone, assigned_to, due_date,
                ...
                <div id="pm-import-preview" style="display: none; margin-top: 24px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                        ...
                        <div style="display: flex; gap: 8px;">
                            <button type="button" class="bntm-btn-small bntm-btn-secondary"
                                onclick="pmSelectAllTasks(true)">Select All</button>
                            <button type="button" class="bntm-btn-small bntm-btn-secondary"
                                onclick="pmSelectAllTasks(false)">Deselect All</button>
                        </div>
                    </div>
                    <div id="pm-tasks-preview-list"
                        style="max-height: 400px; overflow-y: auto; border: 1px solid #e5e7eb; border-radius: 8px; padding: 12px;">
                    </div>
                </div>

                <div class="bntm-modal-footer">
                    <button type="button" class="bntm-btn-secondary" onclick="pmCloseImportModal()">Cancel</button>
                    <button type="button" class="bntm-btn-primary" id="pm-import-btn" onclick="pmImportTasks()"
                        style="display: none;">
    ...
    <div id="pm-google-calendar-modal" class="bntm-modal" style="display: none;">
        <div class="bntm-modal-content" style="max-width: 600px;">
            <div class="bntm-modal-header">
                ...
                <button class="bntm-modal-close"
                    onclick="document.getElementById('pm-google-calendar-modal').style.display = 'none'">&times;</button>
            </div>
            <div class="bntm-form">
                <div
                    style="background: #eff6ff; border-left: 4px solid #0284c7; padding: 16px; border-radius: 6px; margin-bottom: 20px;">
                    <h4 style="margin: 0 0 12px 0; font-size: 16px; color: #0c4a6e;">Quick Setup</h4>
                    <p style="margin: 0; color: #334155; font-size: 14px;">
                        Click the button below to authorize access to your Google Calendar. You'll be redirected to Google
                        to grant permission, then automatically returned here.
                    </p>
                </div>

                <div id="pm-google-auth-status"
                    style="display: none; padding: 12px; border-radius: 6px; margin-bottom: 16px;">
                    <p id="pm-google-auth-message" style="margin: 0;"></p>
                </div>

                <div class="bntm-modal-footer">
                    <button type="button" class="bntm-btn-secondary"
                        onclick="document.getElementById('pm-google-calendar-modal').style.display = 'none'">Close</button>
    ...
    <style>
        ...
        .pm-empty-tasks {
            pointer-events: none;
            /* Allows drag events to pass through the empty state message */
        }
    ...
    <script>
        ...
        let pmTaskTimerInterval = null;
        let pmTaskTimerActiveTaskId = 0;
        ...
        function pmFormatTimerElapsed(startedAt) {
            if (!startedAt) return '00:00:00';
        ...
        function pmStopTaskTimerClock() {
            if (pmTaskTimerInterval) {
                clearInterval(pmTaskTimerInterval);
                pmTaskTimerInterval = null;
            }
        }

        function pmStartTaskTimerClock(startedAt) {
            pmStopTaskTimerClock();
            const elapsedEl = document.getElementById('pm-task-timer-elapsed');
            if (!elapsedEl || !startedAt) return;
            elapsedEl.textContent = pmFormatTimerElapsed(startedAt);
            pmTaskTimerInterval = setInterval(() => {
                elapsedEl.textContent = pmFormatTimerElapsed(startedAt);
            }, 1000);
        }

        function pmRenderTaskTimerButton(taskId, isActive) {
            return `
        <button type="button" class="pm-task-timer-btn pm-task-timer-btn-modal${isActive ? ' is-active' : ''}" data-task-id="${taskId}" data-active="${isActive ? '1' : '0'}">
            <span class="pm-task-timer-btn-dot"></span>
            <span class="pm-task-timer-btn-label">${isActive ? 'Active' : 'Inactive'}</span>
        </button>
    `;
        }

        function pmSyncTaskTimerButtons(activeTaskId) {
            document.querySelectorAll('.pm-task-timer-btn').forEach(btn => {
                const isActive = String(btn.dataset.taskId) === String(activeTaskId) && String(activeTaskId) !== '0';
                btn.dataset.active = isActive ? '1' : '0';
                btn.classList.toggle('is-active', isActive);
                const label = btn.querySelector('.pm-task-timer-btn-label');
                if (label) label.textContent = isActive ? 'Active' : 'Inactive';
            });
        ...
        function pmUpdateTaskTimerUi(taskId, timer) {
            const wrap = document.getElementById('pm-task-timer-button-wrap');
            const status = document.getElementById('pm-task-timer-status');
            const elapsed = document.getElementById('pm-task-timer-elapsed');
            const isActive = !!(timer && timer.is_active);

            pmTaskTimerActiveTaskId = isActive ? taskId : (timer && timer.active_task_id ? parseInt(timer.active_task_id, 10) : 0);
            pmSyncTaskTimerButtons(pmTaskTimerActiveTaskId);

            if (wrap) {
                wrap.innerHTML = pmRenderTaskTimerButton(taskId, isActive);
            }

            if (status) {
                if (isActive) {
                    status.textContent = 'Timer is running. Stop it to automatically save the elapsed time as a time log.';
                } else if (pmTaskTimerActiveTaskId && pmTaskTimerActiveTaskId !== taskId) {
                    status.textContent = 'Another task is currently active. Starting this one will stop the previous timer automatically.';
                } else {
                    status.textContent = 'Start this task to track time automatically.';
                }
            ...
            if (elapsed) {
                if (isActive && timer.started_at) {
                    pmStartTaskTimerClock(timer.started_at);
                } else {
                    pmStopTaskTimerClock();
        ...
        function pmToggleTaskTimer(taskId, forceAction) {
            const trigger = document.querySelector(`.pm-task-timer-btn[data-task-id="${taskId}"]`);
            const isActive = forceAction ? forceAction === 'stop' : (trigger && trigger.dataset.active === '1');
            const action = isActive ? 'pm_stop_task_timer' : 'pm_start_task_timer';

            const formData = new FormData();
            formData.append('action', action);
            formData.append('task_id', taskId);
            if (!isActive) {
                formData.append('client_started_at', new Date().toISOString());
            }
            formData.append('nonce', '<?php echo $nonce; ?>');
            ...
            fetch(ajaxurl, { method: 'POST', body: formData })
                ...
                .then(json => {
                    if (!json.success) {
                        alert(json.data.message || 'Failed to update task timer');
                        return;
                    }

                    if (action === 'pm_start_task_timer') {
                        pmTaskTimerActiveTaskId = taskId;
                        pmSyncTaskTimerButtons(taskId);
                        const modalTaskId = document.getElementById('pm-task-id').value;
                        if (modalTaskId === String(taskId)) {
                            pmUpdateTaskTimerUi(taskId, { is_active: true, active_task_id: taskId, started_at: json.data.started_at });
                        } else if (modalTaskId) {
                            pmUpdateTaskTimerUi(parseInt(modalTaskId, 10), { is_active: false, active_task_id: taskId, started_at: '' });
                        }
                    } else {
                        pmTaskTimerActiveTaskId = 0;
                        pmSyncTaskTimerButtons(0);
                        const modalTaskId = document.getElementById('pm-task-id').value;
                        if (modalTaskId === String(taskId)) {
                            pmUpdateTaskTimerUi(taskId, { is_active: false, active_task_id: 0, started_at: '' });
                            pmEditTask(taskId);
                        } else if (modalTaskId) {
                            pmUpdateTaskTimerUi(parseInt(modalTaskId, 10), { is_active: false, active_task_id: 0, started_at: '' });
                        }
                    }
                });
        }
            ...
            fetch(ajaxurl, { method: 'POST', body: formData })
                ...
                .then(json => {
                    if (json.success) {
                        ...
                        pmUpdateTaskTimerUi(task.id, json.data.timer || {});
                    }
                });
        }
        ...
        document.addEventListener('click', function (e) {
            if (e.target.closest('.pm-task-timer-btn')) {
                const btn = e.target.closest('.pm-task-timer-btn');
                pmToggleTaskTimer(btn.dataset.taskId);
                return;
            }
 ...
 * Render Task Row
 */
function pm_render_task_row($task, $team_members, $custom_statuses, $nonce, $active_task_id = 0)
{
    $assigned_user = $task->assigned_to ? get_userdata($task->assigned_to) : null;
    $is_overdue = $task->due_date && strtotime($task->due_date) < time() && $task->status !== 'completed';
    $is_task_active = (int) $active_task_id === (int) $task->id;
    ...
    <div class="pm-task-row" draggable="true" data-task-id="<?php echo $task->id; ?>"
        ...
        <div class="task-row-status">
            <div class="task-status-dropdown">
                <button type="button" class="task-status-btn pm-task-status-btn"
                    style="background: <?php echo esc_attr($status_color); ?>" data-task-id="<?php echo $task->id; ?>">
                ...
                <div class="task-status-menu">
                    <?php if (!empty($custom_statuses)): ?>
                        <?php foreach ($custom_statuses as $status): ?>
                            <div class="task-status-option pm-task-status-option"
                                ...
                                data-task-id="<?php echo $task->id; ?>" data-status="<?php echo esc_attr($status->status_name); ?>"
                    ...
                    <?php else: ?>
                        <div class="task-status-option pm-task-status-option" style="background: #6b7280"
                            data-task-id="<?php echo $task->id; ?>" data-status="To Do" data-color="#6b7280">
                            To Do
                        </div>
                        <div class="task-status-option pm-task-status-option" style="background: #3b82f6"
                            data-task-id="<?php echo $task->id; ?>" data-status="In Progress" data-color="#3b82f6">
                            In Progress
                        </div>
                        <div class="task-status-option pm-task-status-option" style="background: #f59e0b"
                            data-task-id="<?php echo $task->id; ?>" data-status="Review" data-color="#f59e0b">
                            Review
                        </div>
                        <div class="task-status-option pm-task-status-option" style="background: #10b981"
                            data-task-id="<?php echo $task->id; ?>" data-status="Completed" data-color="#10b981">
                            Completed
                        </div>
                    <?php endif; ?>
                </div>
        ...
        <div class="task-row-actions">
            <?php echo pm_render_task_timer_button($task->id, $is_task_active, 'row'); ?>
            ...
            <button class="bntm-btn-small bntm-btn-danger pm-delete-task-btn" data-task-id="<?php echo $task->id; ?>"
        ...
        <div class="pm-kanban-board">
            <?php foreach ($custom_statuses as $status):
                ...
                <div class="pm-kanban-column" data-status="<?php echo esc_attr($status->status_name); ?>">
                    ...
                    <div class="kanban-column-content sortable-kanban-column"
                        ...
                        <?php else: ?>
                            <?php foreach ($status_tasks as $task):
                                ...
                                $is_overdue = $task->due_date && strtotime($task->due_date) < time() && $task->status !== 'completed';
                                ?>
                                <div class="pm-kanban-card" data-task-id="<?php echo $task->id; ?>" draggable="true"
                                    onclick="pmEditTask(<?php echo $task->id; ?>)">
                ...
                <?php foreach ($team_members as $member):
                    ...
                    <div class="pm-member-card">
                        ...
                        <div class="member-performance">
                            ...
                            <div class="progress-bar">
                                <div class="progress-fill"
                                    style="width: <?php echo $completion_rate; ?>%; background: <?php echo esc_attr($project->color); ?>">
                                </div>
                            </div>
                        </div>
            ...
            <form id="pm-update-project-form" class="bntm-form">
                ...
                <div class="bntm-form-row">
                    ...
                    <div class="bntm-form-group">
                        ...
                        <select name="status">
                            ...
                            <option value="in_progress" <?php selected($project->status, 'In Progress'); ?>>In Progress
                            </option>
                ...
                <div class="bntm-form-group">
                    ...
                    <input type="number" name="budget" step="0.01" min="0"
                        value="<?php echo esc_attr($project->budget); ?>">
                </div>
                ...
                <div class="pm-statuses-list sortable-statuses">
                    <?php foreach ($custom_statuses as $status): ?>
                        <div class="pm-status-item" data-status-id="<?php echo $status->id; ?>" draggable="true">
                            ...
                            <div class="status-color-pReview" style="background: <?php echo esc_attr($status->status_color); ?>">
                            </div>
        ...
        <div class="pm-settings-section danger-zone">
            ...
            <div class="danger-actions">
                <div>
                    ...
                    <p>Once you delete a project, there is no going back. All tasks, milestones, and data will be
...
function bntm_ajax_pm_delete_project()
{
    ...
    if (!empty($task_ids)) {
        ...
        $wpdb->query("DELETE FROM {$wpdb->prefix}pm_active_task_sessions WHERE task_id IN ($task_ids_str)");
    }
...
function bntm_ajax_pm_delete_task()
{
    ...
    $wpdb->delete($wpdb->prefix . 'pm_active_task_sessions', ['task_id' => $task_id]);
...
function bntm_ajax_pm_get_task_details()
{
    ...
    $active_session = pm_get_current_user_active_session();
    ...
    wp_send_json_success([
        ...
        'comments' => $comments,
        'timer' => [
            'is_active' => $active_session && intval($active_session->task_id) === $task_id,
            'active_task_id' => $active_session ? intval($active_session->task_id) : 0,
            'started_at' => $active_session && intval($active_session->task_id) === $task_id ? pm_utc_mysql_to_iso8601($active_session->started_at) : '',
            'current_user_id' => $current_user_id,
        ]
    ]);
}

function bntm_ajax_pm_start_task_timer()
{
    check_ajax_referer('pm_project_detail_nonce', 'nonce');
    ...
    $current_user = wp_get_current_user();
    $task_id = intval($_POST['task_id']);
    $client_started_at = isset($_POST['client_started_at']) ? sanitize_text_field(wp_unslash($_POST['client_started_at'])) : '';
    ...
    if ($existing_session && intval($existing_session->task_id) === $task_id) {
        wp_send_json_success([
            'message' => 'Timer already active for this task',
            'task_id' => $task_id,
            'started_at' => pm_utc_mysql_to_iso8601($existing_session->started_at),
        ]);
    }
    ...
    if ($existing_session) {
        $auto_stop = pm_create_automatic_time_log(
            $existing_session,
            'Automatically stopped when another task was activated'
        );

        if ($auto_stop['success']) {
            $previous_task = $wpdb->get_row($wpdb->prepare(
                "SELECT * FROM {$tasks_table} WHERE id = %d",
            ...
            if ($previous_task) {
                pm_log_activity(
                    $previous_task->project_id,
                    $previous_task->id,
                    $current_user->ID,
                    'stopped_task_timer',
                    'Automatically stopped timer and logged ' . number_format($auto_stop['hours'], 2) . 'h'
                );
            }

            $switched_log = [
                'task_id' => intval($existing_session->task_id),
                'hours' => $auto_stop['hours'],
            ];
        }

        $wpdb->delete($sessions_table, ['id' => $existing_session->id], ['%d']);
    }

    $started_at_utc = pm_parse_client_datetime_to_utc_mysql($client_started_at);
    ...
    if (!$inserted) {
        wp_send_json_error(['message' => 'Failed to start timer']);
    }

    pm_log_activity($task->project_id, $task_id, $current_user->ID, 'started_task_timer', 'Started task timer');

    wp_send_json_success([
        'message' => 'Task timer started',
        'task_id' => $task_id,
        'started_at' => pm_utc_mysql_to_iso8601($started_at_utc),
        'switched_log' => $switched_log,
    ]);
}

function bntm_ajax_pm_stop_task_timer()
{
    check_ajax_referer('pm_project_detail_nonce', 'nonce');
    ...
    $session = $wpdb->get_row($wpdb->prepare(
        "SELECT * FROM {$sessions_table} WHERE user_id = %d AND task_id = %d",
        $current_user->ID,
        $task_id
    ));

    if (!$session) {
        wp_send_json_error(['message' => 'No active timer found for this task']);
    }

    $result = pm_create_automatic_time_log($session, 'Auto logged from active task timer');

    if (!$result['success']) {
        wp_send_json_error(['message' => $result['message']]);
    }

    $wpdb->delete($sessions_table, ['id' => $session->id], ['%d']);
    ...
    if ($task) {
        pm_log_activity(
            ...
            $current_user->ID,
            'stopped_task_timer',
            'Stopped task timer and logged ' . number_format($result['hours'], 2) . 'h'
        );
    }
    ...
    foreach ($tasks as $task) {
        ...
        if (!empty($task['assigned_to'])) {
            ...
            foreach ($team_members as $member) {
                if (
                    strcasecmp($member['display_name'], $assigned_name) === 0 ||
...
function bntm_ajax_pm_add_time_log()
{
    ...
    if ($result) {
        pm_recalculate_task_actual_hours($task_id);
...
function bntm_ajax_pm_delete_time_log()
{
    ...
    if ($result) {
        pm_recalculate_task_actual_hours($log->task_id);
    ...
    <div class="pm-notification-wrapper" style="position: relative; display: inline-block;">
        ...
        <div id="pm-notification-dropdown" class="pm-notification-dropdown" style="display: none;">
            ...
            <div class="pm-notification-list">
                ...
                <?php else: ?>
                    <?php foreach ($notifications as $notification): ?>
                        ...
                        $icon = '📝';
                        if (strpos($notification->action, 'completed') !== false)
                            $icon = '✅';
                        elseif (strpos($notification->action, 'commented') !== false)
                            $icon = '💬';
                        elseif (strpos($notification->action, 'assigned') !== false)
                            $icon = '👤';
                        elseif (strpos($notification->action, 'logged time') !== false)
                            $icon = '⏱️';
                        elseif (strpos($notification->action, 'created') !== false)
                            $icon = '✨';
                        elseif (strpos($notification->action, 'updated') !== false)
                            $icon = '🔄';
                        ...
                        <a href="<?php echo esc_url($link); ?>"
                            class="pm-notification-item <?php echo $is_unread ? 'pm-unread' : ''; ?>">
            ...
            <div class="pm-notification-footer">
                <a href="<?php echo home_url('/projects?tab=overview'); ?>"
