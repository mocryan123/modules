...
function pmLoadJsonFile(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (!tasks) {
                alert('Invalid JSON format. Expected an array of tasks or an object with a tasks array.');
                return;
            }
...
function bntm_ajax_pm_export_tasks() {
    ...
    $filename = 'project-' . $project_id . '-tasks-' . date('Y-m-d') . '.json';

    nocache_headers();
    header('Content-Type: application/json; charset=utf-8');
    header('Content-Disposition: attachment; filename="' . $filename . '"');

    echo wp_json_encode($tasks, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
    exit;
}
